...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《Java模块系统》，属于 Java 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《Java模块系统》纳入自己的知识网络，并与 Java 模块的其他主题（JVM、集合框架、并发、Spring 生态）建立关联。

## 2. 历史动机与发展脉络

《Java模块系统》是 Java 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：Java模块系统 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。

理解 Java 版本与 LTS 机制，是工程选型的起点：生产环境优先 LTS，新特性（如虚拟线程）可以在受控场景评估后引入。

## 3. 形式化定义与核心概念精讲

本节把《Java模块系统》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《Java模块系统》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
该示例来自原文《创建第一个模块》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《编写模块描述文件》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《编写模块代码》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《编译和运行模块》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.7 示例：1. requires 指令详解

该示例来自原文《1. requires 指令详解》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.8 示例：2. exports 指令详解

该示例来自原文《2. exports 指令详解》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.9 示例：3. opens 指令与反射

该示例来自原文《3. opens 指令与反射》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.10 示例：4. 服务机制 uses 和 provides

该示例来自原文《4. 服务机制 uses 和 provides》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.11 示例：4. 服务机制 uses 和 provides
...
该示例共 11 行有效代码，包含 3 类关键结构（class、import、for）。其中：
...
### 5.12 示例：5. 模块与 Maven/Gradle

该示例来自原文《5. 模块与 Maven/Gradle》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：场景一：Spring Boot 应用的模块化

该示例来自原文《场景一：Spring Boot 应用的模块化》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《场景二：库的模块化》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《未导出包的类不可访问》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《反射访问被拒绝》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《jlink 定制运行时》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.19 示例：层（Layer）与模块动态加载

该示例来自原文《层（Layer）与模块动态加载》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.20 示例：module-info.java 声明

该示例来自原文《module-info.java 声明》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《服务声明与使用》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《java 命令运行模块》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《javac 编译模块》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《打包模块 jar》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.38 示例：jlink 创建运行时镜像

该示例来自原文《jlink 创建运行时镜像》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《模块相关 API》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《jdeps 依赖分析》小节，用于演示Java模块系统相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《Java模块系统》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《Java模块系统》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《Java模块系统》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 概述：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 基础概念：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 快速上手：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 详细用法：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 常见场景：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 注意事项与常见错误：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 进阶用法：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- module-info.java 声明：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 服务声明与使用：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- java 命令运行模块：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- javac 编译模块：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 打包模块 jar：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- jlink 创建运行时镜像：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 模块相关 API：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- jdeps 依赖分析：该小节围绕Java模块系统展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 Java 模块。为了把《Java模块系统》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
| Java模块系统 | 044-JavaModuleSystem | 本文自身 |
...
下表整理《Java模块系统》及 Java 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
