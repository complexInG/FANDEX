...
本模块 93 篇文档已按学习顺序重排：前 17 篇是零基础核心（语法、面向对象与异常），第 18-39 篇是核心类库与类型系统，第 40-67 篇是并发、JVM 与 IO/网络/数据库，第 68 篇起是框架与工程实践。零基础推荐路径：
...
第 3 阶段（进阶能力）：051 类加载 → 052 GC → 053 内存模型 → 057 IO 流
    → 065 JDBC → 068 Spring 基础 → 077 设计模式 → 093 图书管理系统项目
```

分层说明：前 39 篇为【核心必读】（语法、OOP、集合、泛型、反射）；第 40-67 篇为【进阶原理】（并发、JVM、IO/网络/数据库）；第 68 篇起为【框架与工程实践】，其中 077 设计模式、085 性能调优、093 项目实战是里程碑；各"语法速查手册"【用到再查】。
