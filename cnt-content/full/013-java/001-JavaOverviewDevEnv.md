...
本模块 93 篇文档已按学习顺序重排：前 17 篇是零基础核心（语法、面向对象与异常），第 18-39 篇是核心类库与类型系统，第 40-64 篇是并发、JVM 与 IO/网络，第 65-66 篇是数据库（JDBC），第 67 篇起是构建工具、设计模式、Spring 与工程实践。零基础推荐路径：
...
第 2 阶段（核心类库）：018 集合框架 → 019 迭代器 → 022 Lambda → 023 Stream
    → 026 日期时间 → 029 泛型详解 → 032 反射 → 040 多线程基础 → 042 JUC
第 3 阶段（进阶能力）：051 类加载 → 052 GC → 053 内存模型 → 057 IO 流
    → 065 数据库 → 066 JDBC → 067 构建工具 → 070 设计模式
    → 071 Spring 基础 → 093 图书管理系统项目
```

分层说明：前 39 篇为【核心必读】（语法、OOP、集合、泛型、反射）；第 40-66 篇为【进阶原理】（并发、JVM、IO/网络/数据库）；第 67 篇起为【框架与工程实践】，其中 070 设计模式、071 Spring 基础、085 性能调优、093 项目实战是里程碑；各"语法速查手册"【用到再查】。
