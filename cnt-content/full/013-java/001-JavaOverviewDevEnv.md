...
本模块 91 篇文档不要按编号顺序读。零基础推荐路径：

```text
第 1 阶段（语法基础）：001 概述 → 002 快速入门 → 003 程序结构 → 004 数据类型
    → 005 变量常量 → 016 运算符 → 027 控制流 → 056 面向对象 → 058 异常
第 2 阶段（核心类库）：049 集合 → 047 数组 → 059 泛型 → 057 抽象类与接口
第 3 阶段（进阶能力）：061 多线程 → 050 并发详解 → 063 Lambda → 064 Stream
第 4 阶段（项目回炉）：069 图书管理系统项目 → 之后按需查速查手册（075-091）
```

分层说明：前两阶段为【核心必读】；JVM 原理（010/011/048/062）、框架（017-020）、性能调优（033）为【进阶原理】；新特性与速查手册【用到再查】。
