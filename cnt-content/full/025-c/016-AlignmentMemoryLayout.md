...
**解析讲解**：伪共享的根源是多个变量位于同一缓存行。`volatile` 不影响内存布局；`atomic` 提供原子性但不改变布局；`#pragma pack(1)` 反而使变量更紧凑，加剧伪共享。只有 `alignas(64)` 能强制每个变量独占缓存行。
...
**解析讲解**：C11 标准建议（非强制）`size` 是 `alignment` 的整数倍。严格实现（如某些 musl libc 版本）会在 `size % alignment != 0` 时失败。最佳实践是向上取整：`aligned_alloc(64, 128)`。
