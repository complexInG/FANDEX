...
## 知识讲解与要点分析（原习题 14.1.1(remember,难度 1)）

在 C 声明 `int *arr[10];` 中,运算符优先级最高的是 ______,因此 arr 的类型是 ______。

**解析讲解**：`[]` / 由 10 个 int 指针构成的数组

**解析讲解**：ISO/IEC 9899:2024 §6.5.1 规定后缀运算符 `[]` 优先级高于前缀运算符 `*`,因此 arr 先与 `[10]` 结合为数组,再与 `*` 结合为指针。

## 知识讲解与要点分析（原习题 14.1.2(remember,难度 1)）

C 声明遵循"______"原则,即声明的形式与使用的形式对称。

**解析讲解**：声明模拟使用(declaration follows use)

**解析讲解**：Dennis Ritchie 在《The Development of the C Language》中阐述:`int *p` 的声明形式与 `*p` 作为表达式的形式对称,这是 C 声明语法的核心设计原则。

## 知识讲解与要点分析（原习题 14.1.3(understand,难度 2)）

`int (*fp)(int);` 中,fp 的类型是 ______。

**解析讲解**：指向接受 int 参数、返回 int 的函数的指针

**解析讲解**：括号 `(*fp)` 使 fp 先与 `*` 结合为指针,再与 `(int)` 结合为函数,最后与 `int` 结合为返回类型。

### 选择题知识点讲解

## 知识讲解与要点分析（原习题 14.2.1(understand,难度 2)）
...
**解析讲解**：A 语法非法(函数不能作为数组元素);C 语法非法(函数不能返回数组);D 语法非法。B 中 func 先与 `[10]` 结合为数组,再与 `*(int)` 结合为函数指针,每个元素是 `int(*)(int)`。

## 知识讲解与要点分析（原习题 14.2.2(analyze,难度 3)）
...
**解析讲解**：`*p` 是 arr[0]=1;`*q[0]` 中,`q` 是 `int(*)[5]`,`q[0]` 是 `int[5]`(数组退化为指针),`*q[0]` 是 `arr[0]`=1;`(*q)[0]` 是 `arr[0]`=1。

## 知识讲解与要点分析（原习题 14.2.3(evaluate,难度 4)）
...
**解析讲解**：原代码声明 func 为"返回 int* 的函数",而非"指向返回 int 的函数的指针"。函数声明与函数指针声明语法不同,需用括号将 `*func` 括起。

## 知识讲解与要点分析（原习题 14.3.2(apply,难度 4)）
...
**解析讲解**：原代码 `int *ops[2]` 是"2 个 int 指针的数组",不是函数指针数组。函数名赋值给 `int*` 产生类型不匹配警告,调用时 UB。修复为 `int (*ops[2])(int, int)`。

## 知识讲解与要点分析（原习题 14.3.3(analyze,难度 4)）
...
## 知识讲解与要点分析（原习题 14.4.2(create,难度 5)）
...
## 知识讲解与要点分析（原习题 14.4.3(analyze,难度 4)）
