...
## 0. 直觉：HTML 搭骨架，CSS 穿衣服

在 HTML5 第一课里说过：HTML 是骨骼，CSS 是衣服。这节课正式学“穿衣服”——让标题变颜色、让卡片有圆角、让页面在手机上不挤。

先建立一个心智模型：CSS 就是“选择器 + 属性”的规则清单。选中谁（选择器），改成什么样（属性与值）。后面所有内容都是这两件事的展开。
...
1. 给 HTML 第一课的博客页面加一个外部样式表：让 `h1` 变蓝色、正文行距 1.6；
2. 用 CSS 变量定义主色与间距，替换样式中的魔法数字；
3. 用 `.card` 类做一张带圆角与阴影的卡片（背景色用 `--card-bg`，不要与页面背景相同）。

### 进阶版（选做）

1. 用 `@media (prefers-color-scheme: dark)` 给页面做深色主题，只覆盖颜色变量；
2. 用 `color-mix()` 或 `filter: brightness()` 实现按钮悬停变暗（不要用 `darken()`）；
3. 用浏览器开发者工具验证优先级：写一个类选择器和一个 ID 选择器，观察哪个生效。
