...
- 架构决策在系统设计中的应用，见 038-software-architecture 模块
- 需求分析方法（写方案前的输入），见 037-software-engineering 模块
- 数据库选型等具体决策案例，见 020-mysql、021-postgresql 模块

> **一句话记忆**：RFC 在决策前"议"（可改），ADR 在决策后"记"（不改），技术方案在实施前"定"（指导代码）——三种文书配合，让"为什么这么做"永远有据可查。
