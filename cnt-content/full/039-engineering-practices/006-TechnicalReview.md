...
1. **设计评审清单**：根据第 4 节的三个维度，结合你所在领域（前端/后端/数据库/运维），定制一份"我的领域专用评审清单"，至少 15 项。

2. **模拟评审**：找一份公开的设计文档（或你写的一份方案），假装自己是评审者：按功能/非功能/工程三个维度各提 3 个问题，并给每个问题标注"必须修改/建议/澄清"。

3. **评审演练**：和同伴互相评审对方的方案。一方讲方案（3 分钟），另一方按清单提问（5 分钟），然后用"三问"技巧（如果 X 发生会怎样 / 为什么不是 A/B / 上线后怎么发现问题）再追问一轮，最后给出明确结论。

## 10. 参考资源

- Google 工程实践文档（评审文化）：https://google.github.io/eng-practices/
- Design Doc 写作与评审指南（Google 内部实践公开）：https://www.industrialempathy.com/posts/design-docs-at-google/
- RFC 评审流程（Rust 社区范例）：https://github.com/rust-lang/rfcs

## 11. 延伸阅读

- 评审前的方案如何写，见本模块《设计文档规范》
- 评审通过后的实现质量如何保证，见本模块《代码审查清单》
- 评审没发现的坑上线后如何处理，见本模块《On-Call最佳实践》《事故复盘方法论》
- 架构评审中的专业判断依据，见 038-software-architecture 模块

> **一句话记忆**：方案评审是软件项目的"体检"——在问题最便宜的阶段发现它；评审的质量取决于三件事：选对评审时机（该审才审）、用对评审维度（功能/非功能/工程三连看）、给对评审结论（明确、可检查、有闭环）。
