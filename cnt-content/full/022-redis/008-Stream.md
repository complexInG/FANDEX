...
压缩节点（iscompr=1）：
+---------------------------------------------------+
| iskey | isnull | iscompr=1 | size=5               |
+---------------------------------------------------+
| 字符: [r][o][m][a][n]                             |
+---------------------------------------------------+
| child_ptr -> 下一个节点                            |
+---------------------------------------------------+
| (value_ptr, 如果 iskey=1)                         |
+---------------------------------------------------+

分叉节点（iscompr=0）：
+---------------------------------------------------+
| iskey | isnull | iscompr=0 | size=3               |
+---------------------------------------------------+
| 字符: [a][u][e]                                   |
+---------------------------------------------------+
| child_ptr_a | child_ptr_u | child_ptr_e           |
+---------------------------------------------------+
| (value_ptr, 如果 iskey=1)                         |
+---------------------------------------------------+
```
...
//
// +----------------------------------------------------------+
// | Master Entry（主条目，存储字段名模板）                    |
// +----------------------------------------------------------+
// | Entry 1（第一条消息，含 ID 增量与字段值）                 |
// +----------------------------------------------------------+
// | Entry 2（第二条消息）                                     |
// +----------------------------------------------------------+
// | ...                                                      |
...
// stream 结构体
// +------------------------------------------------------------------+
// | rax*          | length | last_id | first_id | cgroups* | ...     |
// +------------------------------------------------------------------+
//       |
//       v
// Radix Tree (rax)
...
// +------------------------------------------------------------------+
// | [Master Entry]                                                   |
// |   [count=50][deleted=0][num-fields=3][f1][f2][f3][0]             |
// +------------------------------------------------------------------+
// | [Entry 1]                                                        |
// |   [flags=SAMEFIELDS][ms-delta=0][seq-delta=0]                    |
// |   [v1][v2][v3][lp-count]                                         |
// +------------------------------------------------------------------+
// | [Entry 2]                                                        |
// |   [flags=SAMEFIELDS][ms-delta=0][seq-delta=1]                    |
// |   [v1][v2][v3][lp-count]                                         |
// +------------------------------------------------------------------+
// | ...                                                              |
// +------------------------------------------------------------------+
// | [Entry 50]                                                       |
// |   [flags=SAMEFIELDS][ms-delta=1000][seq-delta=0]                 |
// |   [v1][v2][v3][lp-count]                                         |
// +------------------------------------------------------------------+
```
...
//                       /          \
//                      /            \
//          Group A (last_id=102-0)  Group B (last_id=101-0)
//          /        \                /        \
//    ConsumerA1  ConsumerA2    ConsumerB1  ConsumerB2
//    PEL: [msg4]  PEL: [msg5]   PEL: [msg1,msg3]  PEL: [msg2]
//
// 说明：
// - Group A 已投递到 msg5（last_id=102-0）
//   ConsumerA1 处理 msg4（未确认），ConsumerA2 处理 msg5（未确认）
// - Group B 已投递到 msg3（last_id=101-0）
//   ConsumerB1 处理 msg1 和 msg3（未确认），ConsumerB2 处理 msg2（未确认）
// - 两个组独立消费，互不影响
// - msg4 和 msg5 对 Group B 来说还未投递
```
