...
**题目**：用网格布局实现卡片列表：手机 1 列、平板（md）2 列、桌面（lg）3 列，卡片间距 4。

**提示**：无前缀基础类 + `md:`、`lg:` 递增；用 `gap-4` 控制间距。

**参考答案要点**：

```html
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
  <div class="bg-white rounded-lg p-4 shadow">卡片</div>
  <!-- 重复 6 张卡片 -->
</div>
```
...
**题目**：标题文字在手机端 2xl、平板 3xl、桌面 4xl；正文在手机端 sm、桌面 base。

**提示**：字号类也支持断点前缀；基础字号写在最前。

**参考答案要点**：

```html
<h1 class="text-2xl md:text-3xl lg:text-4xl font-bold">标题</h1>
<p class="text-sm md:text-base">正文</p>
```

### 练习 3：实现 class 策略暗色模式（进阶）

**题目**：把项目暗色模式从"跟随系统"改为"手动切换"：CSS 中重定义 dark 变体，JS 提供切换按钮。

**提示**：`@custom-variant dark (&:where(.dark, .dark *))`；切换 `document.documentElement` 的 `dark` 类。
...
<!-- HTML -->
<html class="dark">
  <div class="bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">主题内容</div>
...
**题目**：做一个按钮：基础蓝底白字；hover 变深；暗色模式下浅蓝底；桌面端（md）加大左右内边距。要求用变体叠加实现。

**提示**：变体顺序自由，从右往左读；`dark:hover:` 表示"暗色下悬停"。

**参考答案要点**：

```html
<button class="bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-400
               text-white rounded-md px-4 py-2 md:px-6">
  组合按钮
</button>
```
...
- Tailwind 官方响应式设计文档：https://tailwindcss.com/docs/responsive-design
- Tailwind 官方暗色模式文档：https://tailwindcss.com/docs/dark-mode
- MDN 媒体查询文档：https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_media_queries
- MDN prefers-color-scheme 文档：https://developer.mozilla.org/zh-CN/docs/Web/CSS/@media/prefers-color-scheme
- Tailwind v4 发布公告（断点与暗色变更）：https://tailwindcss.com/blog/tailwindcss-v4

下一篇《组件复用》将讲解如何把反复出现的工具类组合沉淀为可维护的组件，实现"写一次、用处处"。
