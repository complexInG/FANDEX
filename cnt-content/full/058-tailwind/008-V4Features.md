...
### 练习 1：从 v3 入口迁移到 v4（入门）

**题目**：把下面的 v3 入口 CSS 改写成 v4 写法，并说明 tailwind.config.js 的去留。
...
**提示**：v4 只有一行 `@import`；主题配置转移到 `@theme`。

**参考答案要点**：

```css
@import "tailwindcss";
/* tailwind.config.js 不再需要；主题定制改用 @theme 块 */
```

### 练习 2：安装 v4 + Vite（入门）

**题目**：在一个 Vite + React 项目中安装 Tailwind v4 并接入 `@tailwindcss/vite` 插件。

**提示**：两个包 + vite.config.ts 注册插件 + CSS 一行 import。
...
### 练习 3：用 @source 补充扫描范围（进阶）

**题目**：项目有一个共享组件目录 `../shared-components` 位于入口 CSS 的扫描范围之外，且其中含动态类名配置，请补充扫描配置。

**提示**：`@source` 声明目录；`@source inline()` 强制扫描动态类名来源文件。
...
### 练习 4：迁移主题配置并处理暗色模式（挑战）

**题目**：把一个 v3 项目的 `theme.extend.colors`（含 `primary: '#1677ff'`）和 `darkMode: 'class'` 迁移到 v4。

**提示**：颜色进 `@theme`；暗色策略用 `@custom-variant dark (&:where(.dark, .dark *))`。
...
- Tailwind CSS v4 官方发布公告：https://tailwindcss.com/blog/tailwindcss-v4
- Tailwind 官方升级指南（v3→v4）：https://tailwindcss.com/docs/upgrade-guide
- Tailwind 官方安装文档（Vite）：https://tailwindcss.com/docs/installation/vite
- Tailwind 官方主题变量文档：https://tailwindcss.com/docs/theme
- Tailwind 官方内容配置文档（@source）：https://tailwindcss.com/docs/content-configuration

至此，Tailwind CSS 模块的八篇文章全部完成：从入门概览、安装配置、核心工具类、布局、主题定制、响应式与暗色、组件复用到 v4 新特性，你已经掌握了用 Tailwind 4 搭建完整设计系统的全部能力。
