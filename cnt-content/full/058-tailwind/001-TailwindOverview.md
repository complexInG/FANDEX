...
题目描述：阅读下面的 HTML，说出这段代码实现的样式效果（不需要运行）：

```html
<div class="max-w-md mx-auto rounded-xl bg-white p-6 shadow-lg">
  <h2 class="text-xl font-bold text-gray-900">FANDEX 学习平台</h2>
  <p class="mt-2 text-sm text-gray-500">面向零基础编程学习者</p>
</div>
```

提示：从"宽度、居中、圆角、背景、内边距、阴影"与"字号、字重、颜色"两组角度拆解。

参考答案要点：容器最大宽度 28rem（448px）并在水平方向居中，白色圆角卡片带较大阴影；标题 20px 加粗深灰；正文小号字、灰色、上边距 8px。

### 练习二：把传统 CSS 改写成工具类

题目描述：给定以下传统 CSS 与 HTML，改用 Tailwind 工具类实现完全相同的效果：
...
```html
<div class="notice">本课程需要先完成 CSS 基础模块的学习。</div>
```

提示：颜色可从 amber（琥珀色）色阶中寻找对应值（`amber-100`、`amber-300`、`amber-800`）；圆角 `rounded-lg` 即 8px。

参考答案要点：`<div class="rounded-lg border border-amber-300 bg-amber-100 px-4 py-3 text-sm text-amber-800">本课程需要先完成 CSS 基础模块的学习。</div>`。

### 练习三：响应式卡片对比

题目描述：用工具类实现一个两列卡片：移动端上下堆叠，平板及以上（`md:`）并排显示，并让两张卡片之间保持 16px 间距。

提示：外层用 `grid` + `grid-cols-1 md:grid-cols-2` + `gap-4`；卡片内容可用 `rounded-lg border p-4`。

参考答案要点：外层 `<div class="grid grid-cols-1 gap-4 md:grid-cols-2">`，内部两张 `<div class="rounded-lg border border-gray-200 p-4">`。`gap-4` 统一处理行距与列距。

### 练习四：对比论证

题目描述：用一段话向同学解释"为什么说 Tailwind 比 Bootstrap 更适合定制化产品"，至少给出两个论据。

提示：从"预制组件 vs 原子类"与"定制成本"两个角度展开。

参考答案要点：第一，Bootstrap 的组件带默认外观，定制需要覆写样式、与默认样式"打架"；Tailwind 的工具类无默认外观，组合即设计，没有覆盖成本。第二，Tailwind 只生成用到的类，产物体积小、无死代码；Bootstrap 往往需要引入整库 CSS。
...
- Tailwind UI（官方组件模板）：https://tailwindui.com/
- prettier-plugin-tailwindcss（类名排序插件）：https://github.com/tailwindlabs/prettier-plugin-tailwindcss

延伸阅读建议：下一篇《Tailwind CSS 安装与配置》将手把手带你在 Vite/React/Astro 项目中搭建 Tailwind 4 环境；CSS 基础与变量知识可回看 007-css 模块；本模块后续文档将展开工具类清单与布局系统。
