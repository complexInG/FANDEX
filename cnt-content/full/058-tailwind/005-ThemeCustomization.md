...
**题目**：为一个名为"青柠阅读"的读书网站定义基础令牌：品牌主色（青柠绿 `#52c41a`）、正文色、页面底色、卡片圆角（16px）、卡片阴影。

**提示**：只需一个 `@theme` 块 + 5 个变量；主色建议用 `--color-brand-*` 命名。

**参考答案要点**：

```css
@theme {
  --color-brand-500: #52c41a;
  --color-brand-600: #389e0d;
  --color-text-main: #262626;
  --color-surface: #fafafa;
  --radius-card: 16px;
  --shadow-card: 0 2px 12px rgb(0 0 0 / 0.06);
}
```

### 练习 2：语义化三层令牌（进阶）

**题目**：在练习 1 基础上，把令牌改造成"基础层 + 语义层"结构：语义层引用基础层，并添加"危险色"语义（红色 `#f5222d`）。

**提示**：语义层用 `var()` 引用基础层；验证方式是在 HTML 中使用 `bg-primary`、`text-danger`。
...
**题目**：引入品牌展示字体 `--font-display`，并自定义一个 2.5rem 的标题字号 `--text-hero`（行高 1.2），在页面标题中使用。

**提示**：字号令牌需要同时考虑行高：`--text-hero--line-height`。
...
```html
<h1 class="font-display text-hero">青柠阅读</h1>
```

### 练习 4：运行时品牌切换（挑战）

**题目**：实现一个"一键切换品牌色"的功能：页面上两个按钮分别把主色切成绿色（`#52c41a`）和蓝色（`#1677ff`），所有使用 `bg-primary` 的元素立即变化。

**提示**：用 `document.documentElement.style.setProperty('--color-primary', ...)` 覆盖变量即可，无需重新构建。
...
- OKLCH 颜色函数（MDN）：https://developer.mozilla.org/zh-CN/docs/Web/CSS/color_value/oklch
- Tailwind v4 发布公告：https://tailwindcss.com/blog/tailwindcss-v4

下一篇《响应式与暗色模式》将讲解如何让这套"设计图纸"在不同屏幕与明暗环境下自适应工作。
