...
1. **类图绘制**：为"用户-订单-订单项"设计类图，画出三个类及其关系（注意聚合/组合和多重性）。

2. **时序图绘制**：为"用户下单"画时序图，包含：提交订单 → 校验库存 → 创建订单 → 返回结果。

3. **状态图绘制**：为"订单状态"画状态机图，标出所有合法转换，并找出 2 个"非法转换"（不该存在的路径）。

4. **活动图绘制**：为"登录流程"画活动图（含泳道），包含成功/失败/锁定三个分支。
...
- Martin Fowler《UML 精粹》：https://martinfowler.com/books/uml.html

## 11. 延伸阅读

- 用例图的详细用法，见本模块《需求分析方法》
- 类图在设计模式中的应用，见本模块《设计模式详解》
- 时序图在架构设计中的应用，见 038-software-architecture《软件架构概述》

> **一句话记忆**：UML 是"软件的施工图"——日常用 5 种就够：类图看结构（记住"聚合弱、组合强"）、时序图看对话、活动图看流程、状态机图看状态、用例图看需求；图是活文档，随代码一起维护，价值在沟通不在美观。
