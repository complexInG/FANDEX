...
1. **需求挖掘**：模拟访谈一位"图书管理员"，挖出"图书管理系统"的 5 条需求，每条都要追问到"为什么"。

2. **用户故事**：把下面这条模糊需求改写成 3 条符合 INVEST 的用户故事："用户希望能管理自己的订单。"

3. **用例设计**：为"图书借阅"画用例图（参与者、用例、包含/扩展关系），并写完整的用例描述（含备选流程）。

4. **RTM 练习**：为一个功能（如"用户注册"）建立需求追踪矩阵：需求ID → 设计 → 代码 → 测试 → 状态。
...
- 需求的优先级管理，见本模块《敏捷开发》
- 需求的图形化表达，见本模块《UML图详解》
- 需求到设计的转化，见 038-software-architecture《软件架构概述》

> **一句话记忆**：需求工程是"确保我们造对系统"的过程——从业务需求（为什么）到用户需求（做什么）再到系统需求（怎么做），用用户故事+用例图把需求写清楚，用变更控制+追踪矩阵管理需求变化。
