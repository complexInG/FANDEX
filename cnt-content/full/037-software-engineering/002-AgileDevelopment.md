...
- 敏捷中的代码质量，见本模块《代码重构》《设计模式详解》
- 敏捷的工程实践，见 039-engineering-practices《工程实践概述》

> **一句话记忆**：敏捷是"先做一道菜让客人尝尝再调整"的迭代思维——四大价值（个体互动/可用软件/客户合作/响应变化）是灵魂，Scrum（角色+事件+工件）和 Kanban（可视化+WIP 限制）是两种执行方式；敏捷失败的根源不是工具不对，而是"仪式化"和"没有改进闭环"。
