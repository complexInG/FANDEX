...
> 前置依赖：先读 003 语义化标签。入门必读：第 1-2 章与 4.4 键盘体验；ARIA 组件实现（3.3/6.2）为进阶选读。
...
- 组件模式：`html5/018-WebComponentsPWADevelopment` 中自定义元素如何内置无障碍；
- 表单无障碍：`html5/005-HTML5FormValidation` 的验证提示与 `aria-describedby` 结合；
- 语义基础：先掌握 `html5/003-SemanticTag`，再理解 ARIA 的补充角色；
