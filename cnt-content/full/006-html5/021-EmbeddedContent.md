...
related:
  ...
  - 'html5/023-WebComponentsPWADevelopment'
prerequisites:
...
- 通信进阶：`html5/033-CrossDocumentCommunication` 全面掌握 `postMessage`；
- 安全：CSP 的 `frame-src` 与 `object-src` 指令（见 `css/` 或安全模块）；
- 微前端：`html5/023-WebComponentsPWADevelopment` 对比 iframe 与 Web Components；
- 性能：`html5/036-CriticalRenderingPathAndResourceLoading` 中第三方嵌入对 LCP 的影响；
