...
这是一个把前面多篇知识点串起来的**综合项目**：表单验证（005）、Canvas 签名（006）、本地存储（008）、拖拽（019）、地理定位（020）、通知（008）。适合在学完这些基础后动手实践，不建议一上来就啃完整代码。

推荐路径：先跑通“运行说明”，再对照“关键代码速查”理解每一块，最后按“扩展方向”做自己的改动。
...
（1）表单结构：多步骤表单按“个人信息 → 地址 → 附加信息 → 偏好”分组，每步一个 `<section>`；

（2）验证挂钩：每个输入框带 `required`/`pattern` 等原生验证属性，配合 `aria-describedby` 关联错误提示；

（3）扩展点：签名板 `<canvas>`、文件拖拽区、`data-*` 字段属性为 JavaScript 提供挂载点；

（4）样式与结构分离：`<link>` 引入外部 CSS，`<script>` 延迟加载，HTML 只负责骨架。
...
（1）初始化：`loadDraft()` 从 LocalStorage 恢复草稿，`validateStep()` 校验当前步骤；

（2）签名板：`mousedown`/`mousemove`/`mouseup` 绘制，`toDataURL` 导出签名；

（3）拖拽上传：`dragover` 放行 + `drop` 读取文件，FileReader 预览图片；

（4）持久化：每次输入触发 `saveDraft()`，把表单数据写入 `localStorage`；

（5）反馈：`toast` 轻提示 + Notification 通知，错误与成功都有可见反馈。
...
- 表单基础：`html5/005-HTML5FormValidation`（本项目的验证部分）；
