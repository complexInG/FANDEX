...
> 前置要求：SVG 画图本身零基础可学（形状、路径、渐变都不需要 JS）；涉及交互的示例需要 JavaScript 基础（`javascript/001`-`005`、`027`）。本章末尾的"SVG 与 Canvas 互转"需要先了解 006 的 Canvas 概念，第一遍可跳过。
...
> 第一遍可跳过：本节需要 JavaScript 异步基础（Promise），读完 `javascript/030` 再回来。
