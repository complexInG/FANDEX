...
updated: '2026-08-03'
related:
  - 'html5/004-HTML5CoreGlobalAttributes'
...
- 响应式图片 `<picture>` / `<source>`：见 `html5/018-ImageResponsiveImage`；
- Web Components 的 `<template>` / `<slot>`：见 `html5/023-WebComponentsPWADevelopment`；
- 拖放 `<draggable>` 的完整 API：见 `html5/024-DragAPI`。
