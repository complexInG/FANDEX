...
related:
  - 'html5/016-List'
  - 'html5/017-LinkageAnchor'
  - 'html5/019-AudioVideo'
  - 'html5/020-SVG'
prerequisites:
...
- 格式对比：AVIF/WebP/JPEG 的选择见 `javascript/042-ImageOptimization`（JavaScript 模块）；
- 性能指标：`javascript/048-CoreWebVitalsAndPerformanceMetrics` 中 LCP/CLS 的测量；
- 懒加载原理：`javascript/041-WebAPIBrowserInterface` 中 IntersectionObserver 实现；
- 工程化：`html5/036-CriticalRenderingPathAndResourceLoading` 资源加载策略；
