...
- 交互进阶：`javascript/039-DOMOperationEvent` 全面掌握事件机制；
- 异步提交：`javascript/023-AsyncProgramming` 中 `fetch` 与 `FormData` 的完整用法；
- 后端配合：`sql/` 与 `backend` 模块了解服务端校验与数据存储；
- 无障碍：`html5/009-Accessibility` 中表单与 `aria-describedby` 的规范；
