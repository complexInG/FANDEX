...
updated: '2026-08-02'
related:
  - 'html5/003-HTML5DivSpanContainers'
  - 'html5/006-HTML5BasicContentTags'
  - 'html5/007-HTML5TableAndStructuredContent'
  - 'html5/009-Accessibility'
  - 'html5/010-HTML5FormValidation'
prerequisites: []
---

> 阅读建议（零基础）：本篇是"了解即可"章节。先用好 003-HTML5DivSpanContainers 的 div/span 把页面搭出来，做出第一个完整页面后再回来精读；第一遍重点读第 0 节与第 1 节即可。
...
- 完整实践：阅读 `html5/015-TextSemantic` 与 `html5/016-List` 掌握文本与列表语义；
- 无障碍深化：结合 `html5/009-Accessibility` 学习 WCAG 与 ARIA 完整规范；
- 结构化数据：在 `html5/031-MicrodataJSONLD` 中对比微数据与 JSON-LD；
- 交互组件：`html5/022-ProgressMeter`、`html5/023-WebComponentsPWADevelopment` 可延伸自定义组件语义；
