...
updated: '2026-08-03'
related:
  - 'javascript/044-IndexedDBADatabaseInYourBrowser'
  - 'html5/011-HTML5MultimediaCanvasDrawing'
  - 'html5/012-DocTypeDeclaration'
  - 'html5/014-MetadataCharacterEncoding'
  - 'html5/015-TextSemantic'
prerequisites:
...
一句话判断：**存"几 KB 的设置"用 localStorage；存"几千条需要查询的业务数据"用 IndexedDB。** 完整教程见 `javascript/044-IndexedDBADatabaseInYourBrowser`；与 Service Worker 组合的离线场景见 `html5/027-ServiceWorkerPWA`。
...
- 存储进阶：`javascript/043-StorageForTheWeb` 对比 Cookie/Web Storage/IndexedDB 的完整取舍；
- 离线进阶：`html5/027-ServiceWorkerPWA` 深入 Service Worker 生命周期与缓存策略；
- 通信：`html5/029-WebSocket` 实时数据通道与 `fetch` 的差异；
- 性能：`javascript/048-CoreWebVitalsAndPerformanceMetrics` 中本地缓存对加载指标的影响；
- 安全：`javascript/045-ErrorBoundaryGlobalErrorCatch` 与 CSP 内容安全策略的配合。
