...
- 概念基础：`css/003-CSS3BoxModelDetailed`（盒模型）、`javascript/027-EventLoop`（事件循环与主线程）；
- 指标验证：`javascript/048-CoreWebVitalsAndPerformanceMetrics` 中 LCP/CLS/TBT 的测量；
- 资源加载：`html5/018-ImageResponsiveImage` 中图片的优先级与懒加载；
- 移动性能：`html5/034-ViewportConfigMobileFirst` 中移动优先与视口策略；
...
`css/060-CriticalRenderPathOptimization` 与 `javascript/048-CoreWebVitalsAndPerformanceMetrics` 形成闭环验证。
