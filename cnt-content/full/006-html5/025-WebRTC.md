...
WebRTC 让浏览器之间直接进行音视频通话和数据传输，不需要安装插件——视频会议、直播连麦、点对点文件传输都是它的典型场景。

本篇属于**高级专题**：涉及 NAT 穿透、SDP 信令、ICE 候选等概念，零基础学习者可以先跳过；建议在掌握 JavaScript 异步编程与 WebSocket 之后再回来学习。学习路径：先跑通 4.1 的视频通话 demo，再逐步理解 2、3 章的原理。
...
（1）媒体获取：`getUserMedia` 申请摄像头/麦克风，`srcObject` 把本地流显示到 `<video>`；

（2）连接建立：`RTCPeerConnection` 创建连接，`createOffer`/`createAnswer` 交换 SDP，`addIceCandidate` 交换 ICE 候选；

（3）远端展示：`ontrack` 收到远端媒体轨道后设置到 `remoteVideo.srcObject`；

（4）状态机：通过 `connectionState` 与按钮状态（开始/呼叫/挂断）联动，保证操作顺序。
...
**讲解：** 约束对象可以限定分辨率、帧率与指定设备（`deviceId`）；`applyConstraints` 可动态调整已启动的轨道；`stop()` 关闭轨道并熄灭摄像头指示灯。
