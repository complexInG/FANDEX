...
本模块 32 篇文档不需要按编号顺序读完。0 基础推荐路径：

```text
000 环境搭建 → 001 快速上手 → 002 基础标签 → 003 语义化 → 005 表单
    → 007 DOCTYPE → 009 元数据 → 010 文本语义 → 012 链接
    → 014 音视频 → 027 data-* → 029 viewport → 030 综合项目
```

路径说明：先在 000 打通"编辑器 → 文件 → 浏览器"的闭环，再会写页面（001），接着逐个学标签与组件（002/003/005/007/009/010/012/014/027/029），最后用综合项目串起来（030）。每篇开头的"速通路径"会告诉你哪些章节可跳过。
