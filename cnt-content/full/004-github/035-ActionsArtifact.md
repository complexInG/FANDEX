...
**题目**：写一个工作流：build job 编译出 `dist/` 并上传制品；test job 下载后运行测试；deploy job 下载后执行部署。

**提示**：三段用 `needs` 串联；上传用 `upload-artifact@v4`，下载用 `download-artifact@v4`，名称保持一致。
...
**题目**：3 个系统的矩阵构建都上传制品，请写出不会冲突的命名方式。

**提示**：把矩阵变量拼进 `name`。

**参考答案要点**：`name: build-${{ matrix.os }}`，三个组合分别生成 `build-ubuntu-latest`、`build-macos-latest`、`build-windows-latest`。

### 练习 3：失败时上传调试快照

**题目**：测试失败时把日志和截图上传为制品，成功时不上传（省存储），保留 3 天。

**提示**：`if: failure()`；`retention-days: 3`。
...
**题目**：部署工作流（deploy.yml）监听构建工作流（Build）完成后，下载其制品进行部署。

**提示**：`on: workflow_run` 监听 `Build` 的 `completed`；下载时用 `github.event.workflow_run.id` 作为 `run-id`，并传 `github-token`。
...
- GitHub 官方：存储与共享工作流数据（制品概念、保留期、常见制品类型）：https://docs.github.com/zh/actions/using-workflows/storing-workflow-data-as-artifacts
- actions/upload-artifact 官方仓库（参数详解与示例）：https://github.com/actions/upload-artifact
- actions/download-artifact 官方仓库（跨运行下载说明）：https://github.com/actions/download-artifact
- GitHub 官方：用量限制、计费与管理（制品与日志保留策略）：https://docs.github.com/zh/billing/managing-billing-for-github-actions/about-billing-for-github-actions
- 延伸：制品与缓存的边界，见《Actions 缓存依赖》（033）
- 延伸：制品下载后如何部署到多环境，见《Actions 环境部署》（036）
