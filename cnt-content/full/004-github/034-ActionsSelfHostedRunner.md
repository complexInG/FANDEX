...
### 练习 1：注册一个带 GPU 标签的运行器

**题目**：在一台装有 NVIDIA GPU 的 Linux 服务器上注册自托管运行器，要求带 `gpu` 自定义标签。

**提示**：`./config.sh` 时加 `--labels gpu`；注册成功后工作流用 `runs-on: [self-hosted, gpu]` 派单。

**参考答案要点**：

```bash
./config.sh --url https://github.com/OWNER/REPO --token ABC123 --labels gpu
...
**题目**：同一工作流里，把"构建"派给 Linux 自托管机器、把"测试"派给任意托管机器。

**提示**：不同 job 分别写不同的 `runs-on`。
...
**题目**：工作流可能被 PR 触发，请写出"仅同仓库 PR 才在自托管运行器上执行"的条件。

**提示**：job 级 `if` 判断 PR 来源仓库是否为当前仓库。
...
**题目**：一个 job 配置了 `runs-on: [self-hosted, arm64]` 但始终 queued。列出排查思路。

**提示**：检查是否有在线空闲且标签匹配的运行器；确认注册时的标签；考虑 60 秒接单与 24 小时超时规则。

**参考答案要点**：1) 查看 Settings → Actions → Runners 中该运行器是否 Online；2) 确认其标签是否包含 `arm64`（可用 `--no-default-labels` 情况需自查）；3) 用 `gh api repos/OWNER/REPO/actions/runners` 检查状态；4) 若机器繁忙或启动慢，任务可能 60 秒未接单被重派，需检查资源占用。
...
- GitHub 官方：关于自托管运行器（概念与适用场景）：https://docs.github.com/zh/actions/hosting-your-own-runners/managing-self-hosted-runners/about-self-hosted-runners
- GitHub 官方：添加自托管运行器（注册流程与前置条件）：https://docs.github.com/zh/actions/hosting-your-own-runners/managing-self-hosted-runners/adding-self-hosted-runners
- GitHub 官方：在工作流中使用自托管运行器（标签与组路由）：https://docs.github.com/zh/actions/hosting-your-own-runners/managing-self-hosted-runners/using-self-hosted-runners-in-a-workflow
- GitHub 官方：Actions Runner Controller（Kubernetes 自动扩展）：https://docs.github.com/zh/actions/hosting-your-own-runners/managing-self-hosted-runners/autoscaling-with-self-hosted-runners
- 延伸：自托管运行器的持久缓存优势，见《Actions 缓存依赖》（033）
- 延伸：自托管运行器与生产环境的组合，见《Actions 环境部署》（036）
