...
**题目**：写一个部署 job，引用 `staging` 环境并读取该环境专属的 `DEPLOY_KEY` 密钥。

**提示**：job 级 `environment: staging`；用 `secrets.DEPLOY_KEY` 传入 env。
...
**题目**：写一个工作流，依次部署到 dev、staging、production 三个环境，每段依赖上一段成功。

**提示**：三个 job 分别声明 environment，用 `needs` 串联。
...
**题目**：main 分支部署到 production，其他分支部署到 staging，用一行表达式实现。

**提示**：`environment: ${{ 条件 && 'production' || 'staging' }}`。
...
**题目**：为 production 环境设计一套保护规则，要求：必须 2 位审查者批准、仅 main 分支可部署、同一时间只允许一个部署。请分别说明用哪些功能实现。

**提示**：审查者用 Required reviewers；分支用 Deployment branches；并发用 `concurrency` 关键字。

**参考答案要点**：1) Settings → Environments → production → Required reviewers 配置 2 个审查者（注：规则满足时仅需 1 位批准，若要双人把关可结合禁止自我审查或自定义规则）；2) Deployment branches 选 Selected branches 并填 `main`；3) job 上加 `concurrency: { group: deploy-${{ github.environment }}, cancel-in-progress: false }`。
...
- GitHub 官方：使用环境进行部署（环境概念、保护规则、环境机密）：https://docs.github.com/zh/actions/deployment/targeting-different-environments/using-environments-for-deployment
- GitHub 官方：管理部署环境（创建/删除环境、保护规则细节）：https://docs.github.com/zh/actions/deployment/targeting-different-environments/managing-environments-for-deployment
- GitHub 官方：查看部署历史记录（环境与部署记录）：https://docs.github.com/zh/actions/deployment/managing-your-deployments/viewing-deployment-history
- 延伸：生产环境部署所需的构建产物，见《Actions 制品传递》（035）
- 延伸：生产环境专用自托管运行器，见《Actions 自托管运行器》（034）
