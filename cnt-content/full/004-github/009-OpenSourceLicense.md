...
**题目描述**：你在 GitHub 上创建了一个公开仓库但没有添加许可证。另一位开发者 fork 了你的代码并打包成闭源商业软件。从法律角度分析，他的行为是否合规？

**提示**：回顾第 1 节的原理——没有许可证时的默认状态。

**参考答案要点**：不合规。没有许可证时，默认版权法生效，你保留所有权利，他人不得复制、分发、创作衍生作品。这正是为什么开源项目必须显式添加许可证。（注意：fork 功能本身来自 GitHub 服务条款，与代码使用权是两回事。）

### 练习 2：用决策树选许可证（入门）

**题目描述**：你发布一个 npm 工具库，希望：企业能无障碍使用（包括闭源项目）；同时你担心有人拿它做坏事时怪到你头上。请给出推荐许可证及理由。

**提示**：工具库 + 广泛采用 → 宽松派；免责声明所有许可证都包含。

**参考答案要点**：推荐 MIT。理由：MIT 允许商业闭源使用（降低采用门槛），仅要求保留版权声明；MIT 文本自带"按现状提供、不作担保"的免责条款，可降低作者责任风险。

### 练习 3：比较 Apache 与 MIT（进阶）

**题目描述**：你在企业工作，要发布一个涉及核心算法的库。公司法务要求"使用者不能反过来告我们专利侵权"，同时要求"修改我们代码的人必须声明变更"。选择哪个许可证？

**提示**：对照第 2 节总表，查找同时满足"专利授权 + 变更声明"的许可证。

**参考答案要点**：Apache 2.0。它包含明确的专利授权条款（使用者获得专利许可，不能反向起诉专利侵权），并要求修改衍生作品时在 NOTICE 中声明变更。

### 练习 4：识别 GPL 传染场景（进阶）

**题目描述**：你写了一个使用 GPL 库的桌面软件。请判断以下三种场景是否必须开源你的代码：(1) 仅内部使用；(2) 免费分发给用户；(3) 部署为在线服务供用户访问。

**提示**：GPL 的义务触发条件是"分发"；AGPL 才把网络服务算作分发。

**参考答案要点**：(1) 内部使用不分发，无开源义务；(2) 分发触发 GPL 传染，必须开源；(3) GPL 的"网络服务"不算分发，无开源义务（但若换成 AGPL 库，则必须开源）。

### 练习 5：为仓库添加许可证并验证（综合）

**题目描述**：为一个已有仓库添加 MIT 许可证，并通过 GitHub 搜索验证许可证被正确识别。

**提示**：Web 界面 "Create new file" 输入 `LICENSE` 触发模板按钮；提交后用 `license:mit` 搜索或查看仓库首页徽章。

**参考答案要点**：1. Add file → Create new file；2. 文件名输入 `LICENSE`，点击 "Choose a license template" 选 MIT；3. 确认年份与用户名后提交；4. 在仓库首页看到许可证徽章；5. 用搜索限定符 `license:mit` 验证可检索到该仓库。
