...
**题目**：为你的项目创建一个工作流，要求只在 `main` 分支的 push 时运行 `npm ci && npm test`。

**提示**：文件放在 `.github/workflows/ci.yml`；触发器用 `on: push` 加 `branches` 过滤；job 用 `runs-on: ubuntu-latest`，先 `actions/checkout@v4` 再执行命令。
...
**题目**：让工作流只在 PR 的改动涉及 `src/**` 或 `tests/**` 时运行 lint 和测试，避免只改 README 也跑 CI。

**提示**：触发器用 `on: pull_request`，配合 `paths` 过滤；`types` 建议 `[opened, synchronize]`。
...
**题目**：写一个工作流，每天 UTC 02:00 自动跑一次依赖安全扫描，同时允许手动触发（可以传入"是否发送通知"的布尔参数）。

**提示**：`on` 下同时写 `schedule` 与 `workflow_dispatch`；手动参数用 `inputs` 定义，job 里用 `github.event.inputs.xxx` 读取。
...
### 练习 4：发布 Release 时自动发版

**题目**：当仓库发布一个以 `v` 开头的 Release 时，自动构建并输出版本号（可用于后续推送到 npm/镜像仓库）。

**提示**：用 `on: release` 的 `published` 类型；版本号从 `github.ref`（形如 `refs/tags/v1.2.3`）中提取。
...
- GitHub 官方：触发工作流的事件（完整事件清单）：https://docs.github.com/zh/actions/using-workflows/events-that-trigger-workflows
- GitHub 官方：触发工作流（触发原理与三步流程）：https://docs.github.com/zh/actions/using-workflows/triggering-a-workflow
- GitHub 官方：工作流语法参考（`on` 关键字完整语法）：https://docs.github.com/zh/actions/using-workflows/workflow-syntax-for-github-actions
- GitHub 官方：自动令牌身份验证（GITHUB_TOKEN 与递归触发限制）：https://docs.github.com/zh/actions/security-guides/automatic-token-authentication
- 延伸：触发事件之后如何组织 job 并行/串行，见《GitHub Actions 与 CI/CD》（029）与《Actions 矩阵构建》（032）
- 延伸：工作流运行记录与 `gh run` 命令操作，见 004-github 模块相关文档
