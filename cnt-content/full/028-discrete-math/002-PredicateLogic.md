...
- 谓词逻辑在集合中的应用，见本模块《集合与关系》
- 逻辑在数据库查询中的应用，见 019-sql 模块

> **一句话记忆**：谓词逻辑 = 命题逻辑 + 谓词（性质/关系）+ 量词（$\forall$ 所有 / $\exists$ 存在）——它能表达"所有人都会死"这种量化命题；记住三个易错点：**"所有 A 是 B" 用 $\to$、量词顺序不能乱换、先 EI 再 UI**。
