...
**解析讲解**：C。`string` 是引用类型（不可变）。

**常见疑问 2**：`int? x = null;` 的底层类型是？
- A. `int`
- B. `object`
- C. `Nullable<int>`
- D. `int*`

**解析讲解**：C。`T?` 等价于 `Nullable<T>`。
...
**解析讲解**：C。赋值给 `object` 触发装箱。

**常见疑问 4**：`0.1 + 0.2 == 0.3` 在 C# 中结果为？
- A. `true`
- B. `false`
- C. 编译错误
- D. 运行时异常
...
**常见疑问 5**：以下哪个 C# 版本引入了 `switch` 表达式？
- A. C# 7.0
- B. C# 8.0
- C. C# 9.0
- D. C# 10.0
...
**解析讲解**：A。编译期常量字符串会被实习（interned），引用相同。

**常见疑问 7**：以下代码哪个会编译警告（开启 NRT）？
- A. `string s = null;`
- B. `string? s = null;`
- C. `var s = null;`
- D. `string s = "";`

**解析讲解**：A 与 C。`string` 不可空，赋值 `null` 警告；`var s = null` 类型推断为 `object?` 警告。
