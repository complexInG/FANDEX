...
1. **动手实验**：按第 7 节的三个实验，在你的电脑上对比 npm 与 pnpm 的 node_modules 结构（`ls -la node_modules` 观察符号链接）。

2. **概念辨析**：用自己的话解释"内容寻址存储""符号链接""严格依赖隔离"三个机制分别解决了什么问题。

3. **故障排查**：某项目 CI 报 `Cannot find module 'x'`，但本地能跑。分析可能原因，并说明如何用 pnpm 的思想避免这种问题。
