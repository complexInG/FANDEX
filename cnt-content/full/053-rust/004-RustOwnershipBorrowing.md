...
1. **移动实验**：写代码验证 `let s2 = s1;` 后 s1 不可用，而 `let b = a;`（整数）后 a 仍可用，解释差异原因。

2. **借用规则**：写一个函数接收 `&String` 返回长度；再写一个接收 `&mut String` 追加内容；观察借用规则如何生效。

3. **切片练习**：实现 `first_word`，并用它处理"字符串+字面量"两种输入，验证 `&str` 参数的通用性。

4. **生命周期**：把 `longest` 函数抄写一遍，尝试删除 `'a` 标注，观察编译器报错并理解其含义。

5. **综合应用**：写一个"统计文本中元音字母数量"的函数，参数用 `&str`，用迭代器实现，全程无分配。
