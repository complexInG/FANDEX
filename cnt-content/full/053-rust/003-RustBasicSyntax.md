...
1. **温度转换**：写一个函数把摄氏温度转换为华氏温度（`F = C × 9/5 + 32`），并写 `main` 调用验证。

2. **斐波那契**：用循环（`for`）生成前 20 个斐波那契数，存入数组并打印。

3. **遮蔽实验**：写一段使用变量遮蔽逐步变换数值的代码（如整数 → 加 1 → 转字符串），理解遮蔽与 `mut` 的区别。

4. **九九乘法表**：用嵌套 `for` 循环打印九九乘法表。

5. **类型练习**：分别声明 i8、u32、f64、char 变量，测试溢出（`200u8 + 100`）在 debug 与 release 下的不同行为。
