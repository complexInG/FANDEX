...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《Go 函数与方法》，属于 Go 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《Go 函数与方法》纳入自己的知识网络，并与 Go 模块的其他主题（goroutine、channel、内存模型、标准库）建立关联。

## 2. 历史动机与发展脉络

《Go 函数与方法》是 Go 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：Go 函数与方法 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《Go 函数与方法》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《Go 函数与方法》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
该示例来自原文《1.1 基本语法》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.2 多返回值》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.3 命名返回值》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.4 可变参数》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 17 行有效代码，包含 3 类关键结构（func、for、return）。其中：
...
该示例来自原文《1.5 函数是一等公民》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.6 闭包》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2. init 函数》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 方法定义》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 17 行有效代码，包含 2 类关键结构（func、return）。其中：
...
### 5.10 示例：3.2 值接收者 vs 指针接收者

该示例来自原文《3.2 值接收者 vs 指针接收者》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 23 行有效代码，包含 2 类关键结构（func、return）。其中：
...
### 5.11 示例：3.3 任何类型都可以有方法

该示例来自原文《3.3 任何类型都可以有方法》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 18 行有效代码，包含 2 类关键结构（func、return）。其中：
...
### 5.12 示例：3.4 方法值与方法表达式

该示例来自原文《3.4 方法值与方法表达式》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.1 接口定义》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.2 隐式实现》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.3 接口组合》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.4 空接口》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.17 示例：4.5 类型断言与类型开关

该示例来自原文《4.5 类型断言与类型开关》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 19 行有效代码，包含 4 类关键结构（class、func、if、return）。其中：
...
### 5.18 示例：5.1 函数选项模式（Functional Options）

该示例来自原文《5.1 函数选项模式（Functional Options）》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 32 行有效代码，包含 3 类关键结构（func、for、return）。其中：
...
该示例来自原文《5.2 中间件模式》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 19 行有效代码，包含 2 类关键结构（func、return）。其中：
...
该示例来自原文《函数定义》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 4 行有效代码，包含 2 类关键结构（func、return）。其中：
...
该示例来自原文《多返回值》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《命名返回值》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 6 行有效代码，包含 2 类关键结构（func、return）。其中：
...
该示例来自原文《可变参数》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《函数作为一等公民》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《闭包》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《init 函数》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《方法定义》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《自定义类型方法》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《方法值与方法表达式》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《接口定义与实现》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《接口组合》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《空接口》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《类型断言与类型开关》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《函数选项模式》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《中间件模式》小节，用于演示Go 函数与方法相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《Go 函数与方法》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《Go 函数与方法》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《Go 函数与方法》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. 函数定义：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. init 函数：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 方法与接收者：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 接口与隐式实现：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. 函数设计模式：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 函数定义：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 多返回值：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 命名返回值：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 可变参数：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 函数作为一等公民：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 闭包：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- init 函数：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 方法定义：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 自定义类型方法：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 方法值与方法表达式：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 接口定义与实现：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 接口组合：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 空接口：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 类型断言与类型开关：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 函数选项模式：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 中间件模式：该小节围绕Go 函数与方法展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 Go 模块。为了把《Go 函数与方法》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
| Go 函数与方法 | 003-GoFunctionMethod | 本文自身 |
...
下表整理《Go 函数与方法》及 Go 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
