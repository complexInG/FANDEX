...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《JUnit5》，属于 软件测试 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《JUnit5》纳入自己的知识网络，并与 软件测试 模块的其他主题（测试分层、用例设计、自动化、质量度量）建立关联。

## 2. 历史动机与发展脉络

《JUnit5》是 软件测试 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：JUnit5 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《JUnit5》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《JUnit5》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
该示例来自原文《1.2 Maven 依赖》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 标准断言》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.2 分组断言》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.3 超时断言》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4. 生命周期》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5.1 基本参数化》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5.3 CSV 参数化》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.8 示例：5.4 MethodSource

该示例来自原文《5.4 MethodSource》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6. 嵌套测试》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《7.1 自定义扩展》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《@Test 注解》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.12 示例：@DisplayName 显示名

该示例来自原文《@DisplayName 显示名》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：@BeforeEach / @AfterEach

该示例来自原文《@BeforeEach / @AfterEach》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.14 示例：@BeforeAll / @AfterAll

该示例来自原文《@BeforeAll / @AfterAll》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《生命周期执行顺序》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.16 示例：@Disabled 禁用测试

该示例来自原文《@Disabled 禁用测试》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《@Tag 标签过滤》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《@Nested 嵌套测试》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.19 示例：@RepeatedTest 重复测试

该示例来自原文《@RepeatedTest 重复测试》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.20 示例：@ParameterizedTest 参数化

该示例来自原文《@ParameterizedTest 参数化》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.21 示例：@ValueSource 值来源

该示例来自原文《@ValueSource 值来源》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.22 示例：@CsvSource CSV 来源

该示例来自原文《@CsvSource CSV 来源》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.23 示例：@MethodSource 方法来源

该示例来自原文《@MethodSource 方法来源》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.24 示例：AssertAll 分组断言

该示例来自原文《AssertAll 分组断言》小节，用于演示JUnit5相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《JUnit5》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《JUnit5》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《JUnit5》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. JUnit 5 概述：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. 常用注解：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 断言：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 生命周期：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. 参数化测试：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. 嵌套测试：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 7. 扩展模型：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 8. 最佳实践：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @Test 注解：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @DisplayName 显示名：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @BeforeEach / @AfterEach：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @BeforeAll / @AfterAll：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 生命周期执行顺序：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @Disabled 禁用测试：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @Tag 标签过滤：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @Nested 嵌套测试：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @RepeatedTest 重复测试：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @ParameterizedTest 参数化：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @ValueSource 值来源：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @CsvSource CSV 来源：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- @MethodSource 方法来源：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- AssertAll 分组断言：该小节围绕JUnit5展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 软件测试 模块。为了把《JUnit5》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《JUnit5》及 软件测试 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
