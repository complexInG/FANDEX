...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《控制流》，属于 Python 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《控制流》纳入自己的知识网络，并与 Python 模块的其他主题（数据类型、函数、模块、异常、并发）建立关联。

## 2. 历史动机与发展脉络

《控制流》是 Python 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
### 5.1 示例：1.1 `if-elif-else` 语句

该示例来自原文《1.1 `if-elif-else` 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 31 行有效代码，包含 1 类关键结构（if）。其中：
...
### 5.2 示例：1.2 三元表达式 (Ternary Expression)

该示例来自原文《1.2 三元表达式 (Ternary Expression)》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.3 示例：1.3 `match-case` 语句 (Python 3.10+)

该示例来自原文《1.3 `match-case` 语句 (Python 3.10+)》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 66 行有效代码，包含 2 类关键结构（class、def）。其中：
...
### 5.5 示例：2.1.2 使用 `range()` 函数

该示例来自原文《2.1.2 使用 `range()` 函数》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.6 示例：2.1.3 使用 `enumerate()` 函数

该示例来自原文《2.1.3 使用 `enumerate()` 函数》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.7 示例：2.1.4 使用 `zip()` 函数

该示例来自原文《2.1.4 使用 `zip()` 函数》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 16 行有效代码，包含 2 类关键结构（from、for）。其中：
...
该示例来自原文《2.2 `while` 循环》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 26 行有效代码，包含 2 类关键结构（if、while）。其中：
...
### 5.9 示例：2.3.1 `break` 语句

该示例来自原文《2.3.1 `break` 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 15 行有效代码，包含 3 类关键结构（if、for、while）。其中：
...
### 5.10 示例：2.3.2 `continue` 语句

该示例来自原文《2.3.2 `continue` 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 11 行有效代码，包含 2 类关键结构（if、for）。其中：
...
### 5.11 示例：2.3.3 `pass` 语句

该示例来自原文《2.3.3 `pass` 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 11 行有效代码，包含 4 类关键结构（def、function、if、for）。其中：
...
### 5.12 示例：2.4 `for-else` 和 `while-else` 语句

该示例来自原文《2.4 `for-else` 和 `while-else` 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 30 行有效代码，包含 5 类关键结构（def、if、for、while、return）。其中：
...
### 5.13 示例：3. 异常处理 (Exception Handling)

该示例来自原文《3. 异常处理 (Exception Handling)》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《if 条件语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《三元条件表达式》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.19 示例：match-case 语句

该示例来自原文《match-case 语句》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 14 行有效代码，包含 2 类关键结构（class、def）。其中：
...
该示例共 7 行有效代码，包含 2 类关键结构（if、while）。其中：
...
该示例来自原文《无限循环》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.48 示例：循环中的 else 与 break

该示例来自原文《循环中的 else 与 break》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《迭代器与可迭代对象》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《生成器表达式》小节，用于演示控制流相关操作。阅读时请先看代码结构，再看其后的讲解。
...
- 1. 条件分支 (Selection)：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. 循环结构 (Iteration)：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 异常处理 (Exception Handling)：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 控制流的最佳实践：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- if 条件语句：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 三元条件表达式：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- match-case 语句：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
- 无限循环：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 循环中的 else 与 break：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 迭代器与可迭代对象：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 生成器表达式：该小节围绕控制流展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 Python 模块。为了把《控制流》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《控制流》及 Python 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
