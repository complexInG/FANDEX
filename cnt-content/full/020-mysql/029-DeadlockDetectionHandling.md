...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《死锁检测与处理》，属于 MySQL 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《死锁检测与处理》纳入自己的知识网络，并与 MySQL 模块的其他主题（InnoDB、索引、日志、主从、性能调优）建立关联。

## 2. 历史动机与发展脉络

《死锁检测与处理》是 MySQL 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
该示例来自原文《1.1 自动检测》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：1.2 等图（Wait-for Graph）

该示例来自原文《1.2 等图（Wait-for Graph）》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2. 死锁日志》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 固定顺序访问》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.2 缩短事务》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.3 降低隔离级别》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.4 添加合适索引》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4. 死锁恢复》小节，用于演示死锁检测与处理相关操作。阅读时请先看代码结构，再看其后的讲解。
...
- 1. 死锁检测：该小节围绕死锁检测与处理展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. 死锁日志：该小节围绕死锁检测与处理展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 死锁预防：该小节围绕死锁检测与处理展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 死锁恢复：该小节围绕死锁检测与处理展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 MySQL 模块。为了把《死锁检测与处理》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《死锁检测与处理》及 MySQL 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
...
《死锁检测与处理》属于 MySQL 模块。要理解它，先要理解它在模块中的位置：它解决的是该领域的一个具体问题，并依赖模块内若干前置概念；反过来，它又为后续进阶主题提供基础。
