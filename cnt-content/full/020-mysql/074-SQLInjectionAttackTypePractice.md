...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《SQL 注入攻击类型与实战》，属于 MySQL 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《SQL 注入攻击类型与实战》纳入自己的知识网络，并与 MySQL 模块的其他主题（InnoDB、索引、日志、主从、性能调优）建立关联。

## 2. 历史动机与发展脉络

《SQL 注入攻击类型与实战》是 MySQL 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：SQL 注入攻击类型与实战 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《SQL 注入攻击类型与实战》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《SQL 注入攻击类型与实战》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
### 5.1 示例：1.1.1 基于错误的注入（Error-based）

该示例来自原文《1.1.1 基于错误的注入（Error-based）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：1.1.1 基于错误的注入（Error-based）
...
### 5.3 示例：1.1.2 UNION 查询注入

该示例来自原文《1.1.2 UNION 查询注入》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.4 示例：1.1.3 堆叠查询注入（Stacked Queries）

该示例来自原文《1.1.3 堆叠查询注入（Stacked Queries）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.5 示例：1.2.1 布尔盲注（Boolean Blind）

该示例来自原文《1.2.1 布尔盲注（Boolean Blind）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.6 示例：1.2.1 布尔盲注（Boolean Blind）
...
该示例共 26 行有效代码，包含 5 类关键结构（def、import、if、for、return）。其中：
...
### 5.7 示例：1.2.2 时间盲注（Time-based）

该示例来自原文《1.2.2 时间盲注（Time-based）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 31 行有效代码，包含 5 类关键结构（def、import、if、for、return）。其中：
...
### 5.9 示例：1.3 二次注入（Second-order Injection）

该示例来自原文《1.3 二次注入（Second-order Injection）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 13 行有效代码，包含 5 类关键结构（def、return、SELECT、INSERT、FROM）。其中：
...
### 5.10 示例：1.4 宽字节注入（Wide Byte Injection）

该示例来自原文《1.4 宽字节注入（Wide Byte Injection）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.11 示例：1.6 带外注入（Out-of-band Injection）

该示例来自原文《1.6 带外注入（Out-of-band Injection）》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.1.2 危险代码》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：2.1.3 攻击 Payload

该示例来自原文《2.1.3 攻击 Payload》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.14 示例：2.1.4 执行的 SQL

该示例来自原文《2.1.4 执行的 SQL》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.15 示例：2.1.6 其他 Payload 变体

该示例来自原文《2.1.6 其他 Payload 变体》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.2.2 危险代码》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 5 行有效代码，包含 4 类关键结构（def、return、SELECT、FROM）。其中：
...
该示例来自原文《2.2.3 攻击步骤》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.3.2 攻击脚本》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 23 行有效代码，包含 5 类关键结构（def、import、if、for、return）。其中：
...
该示例来自原文《2.4.1 攻击脚本》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 32 行有效代码，包含 5 类关键结构（def、import、if、for、return）。其中：
...
该示例来自原文《2.5.2 攻击步骤》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.26 示例：3.1.1 创建测试数据库

该示例来自原文《3.1.1 创建测试数据库》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 30 行有效代码，包含 1 类关键结构（CREATE）。其中：
...
### 5.27 示例：3.1.2 创建 Vulnerable Web 应用

该示例来自原文《3.1.2 创建 Vulnerable Web 应用》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 38 行有效代码，包含 7 类关键结构（def、import、from、if、return、SELECT）。其中：
...
### 5.28 示例：3.2.1 练习 1：绕过登录

该示例来自原文《3.2.1 练习 1：绕过登录》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.29 示例：3.2.2 练习 2：UNION 查询

该示例来自原文《3.2.2 练习 2：UNION 查询》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 1 行有效代码，包含 1 类关键结构（SELECT）。其中：
...
### 5.30 示例：3.2.3 练习 3：获取用户数据

该示例来自原文《3.2.3 练习 3：获取用户数据》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.31 示例：3.2.4 练习 4：时间盲注

该示例来自原文《3.2.4 练习 4：时间盲注》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.3 修复演练》小节，用于演示SQL 注入攻击类型与实战相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 29 行有效代码，包含 5 类关键结构（def、if、return、SELECT、FROM）。其中：
...
对比是理解《SQL 注入攻击类型与实战》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《SQL 注入攻击类型与实战》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《SQL 注入攻击类型与实战》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. SQL 注入攻击类型 (Attack Types)：该小节围绕SQL 注入攻击类型与实战展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. SQL 注入实战案例 (Practical Cases)：该小节围绕SQL 注入攻击类型与实战展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 实战演练 (Hands-on Practice)：该小节围绕SQL 注入攻击类型与实战展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 MySQL 模块。为了把《SQL 注入攻击类型与实战》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《SQL 注入攻击类型与实战》及 MySQL 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
