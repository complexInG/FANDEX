...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《JSON类型与JSON-TABLE》，属于 MySQL 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《JSON类型与JSON-TABLE》纳入自己的知识网络，并与 MySQL 模块的其他主题（InnoDB、索引、日志、主从、性能调优）建立关联。

## 2. 历史动机与发展脉络

《JSON类型与JSON-TABLE》是 MySQL 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：JSON类型与JSON-TABLE 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《JSON类型与JSON-TABLE》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《JSON类型与JSON-TABLE》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
### 5.1 示例：1.1 JSON vs JSON 文本

该示例来自原文《1.1 JSON vs JSON 文本》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：1.2 JSON 插入与校验

该示例来自原文《1.2 JSON 插入与校验》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.1 提取函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.4 示例：2.2 JSON_PATH 语法

该示例来自原文《2.2 JSON_PATH 语法》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.3 修改函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.4 查询与搜索函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 基本语法》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.2 展开对象数组》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 28 行有效代码，包含 4 类关键结构（SELECT、INSERT、CREATE、FROM）。其中：
...
### 5.10 示例：3.3 嵌套展开（NESTED PATH）

该示例来自原文《3.3 嵌套展开（NESTED PATH）》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 29 行有效代码，包含 3 类关键结构（SELECT、INSERT、FROM）。其中：
...
### 5.11 示例：3.4 ORDINALITY 列

该示例来自原文《3.4 ORDINALITY 列》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.5 处理缺失值》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.1 虚拟列索引》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 7 行有效代码，包含 3 类关键结构（SELECT、ALTER、FROM）。其中：
...
### 5.14 示例：4.2 函数索引（MySQL 8.0+）

该示例来自原文《4.2 函数索引（MySQL 8.0+）》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.15 示例：4.3 多值索引（MySQL 8.0.17+）

该示例来自原文《4.3 多值索引（MySQL 8.0.17+）》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《JSON 数据类型》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《JSON 提取函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《JSON 修改函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.32 示例：JSON 查询与搜索函数

该示例来自原文《JSON 查询与搜索函数》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《JSON_TABLE》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《JSON 索引优化》小节，用于演示JSON类型与JSON-TABLE相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《JSON类型与JSON-TABLE》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《JSON类型与JSON-TABLE》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《JSON类型与JSON-TABLE》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. JSON 数据类型：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. JSON 查询函数：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. JSON_TABLE：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. JSON 索引优化：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON 数据类型：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON 提取函数：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON 修改函数：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON 查询与搜索函数：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON_TABLE：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- JSON 索引优化：该小节围绕JSON类型与JSON-TABLE展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 MySQL 模块。为了把《JSON类型与JSON-TABLE》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《JSON类型与JSON-TABLE》及 MySQL 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
