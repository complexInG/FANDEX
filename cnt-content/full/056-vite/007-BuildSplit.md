...
**题目**：创建一个新项目，把 `echarts` 装进依赖并在 `main.ts` 里 `import * as echarts from 'echarts'`，执行 `pnpm build`，观察终端输出。回答：警告信息是什么？`index.js` 大概多大？

**提示**：echarts 全量引入约 1MB+；对比只引入单个图表模块（如 `echarts/core` + 所需图表）后的体积。

**参考答案要点**：全量引入时终端出现 `Some chunks are larger than 500 kB after minification` 警告，index chunk 超过 1MB。改为按需引入 `echarts/core`、`echarts/charts`、`echarts/components` 后，体积显著下降——这也是"按需引入"的价值演示。

### 练习 2：实现路由级懒加载（入门）

**题目**：用 React 或 Vue 写一个三页面的小应用（Home / About / Contact），要求每个页面独立成 chunk，验证 Network 面板中切换路由时才加载对应 JS。

**提示**：React 用 `React.lazy` + `Suspense`；Vue 用 `component: () => import(...)`。构建后检查 `dist/assets` 下是否有三个独立文件。
...
构建后 `dist/assets/` 出现 `Home-xxx.js`、`About-xxx.js`、`Contact-xxx.js`；Network 面板里初次只加载当前路由对应的 chunk。

### 练习 3：manualChunks 函数式分组（进阶）

**题目**：项目依赖 `react`、`react-dom`、`antd`、`lodash-es`、`axios`。用函数形式的 `manualChunks` 把它们分成三组：框架组、UI 组、工具组，并解释为什么"框架组"和"工具组"要分开。

**提示**：变更频率是分组的第一原则——框架几乎不变，UI 库跟随版本升级，工具库可能频繁换（如 dayjs 换 moment）。
...
框架升级频率最低放一组，可享最长缓存；UI 和工具库独立，某组升级只重下对应 chunk，互不影响。

### 练习 4：产物体积可视化分析（进阶）

**题目**：给项目接入 `rollup-plugin-visualizer`，执行构建并打开分析页。找出体积最大的前三个依赖，并给出每个的优化建议。

**提示**：treemap 中看最大的矩形；常见优化：按需引入、换轻量替代库（如 `dayjs` 替代 `moment`）、路由懒加载。

**参考答案要点**：安装并配置 `visualizer({ open: true })` 后，`pnpm build` 自动打开 `dist/stats.html`。按矩形面积排序定位大依赖；对策：图表库按模块引入、UI 库开启按需、moment 换 dayjs 等。

### 练习 5（挑战）：把首屏体积降 50%

**题目**：给定一个"全量引入 echarts + antd + 无任何懒加载"的项目，你只有三次配置/代码改动机会，设计一套优化方案，并估算每步的收益。写出完整的 `vite.config.ts` 与改动清单。

**提示**：优先级从高到低：动态 import 拆路由 → 按需引入大库 → manualChunks 分组 → 检查 tree-shaking。

**参考答案要点**：第一步，所有路由改 `lazy(() => import(...))`（首屏不再包含非当前页代码）；第二步，echarts/antd 改为按需引入（`echarts/core`、`antd` 的按需导入）；第三步，manualChunks 函数式分组确保缓存复用。三步叠加通常能把首屏 gzip 体积降低 50% 以上，再用 visualizer 验证。
...
- Vite 构建选项（官方中文）：https://cn.vitejs.dev/config/build-options
- Vite 部署指南（官方中文）：https://cn.vitejs.dev/guide/static-deploy
- Vite 构建与代码分割指南（英文）：https://vite.dev/guide/build
- Rollup output.manualChunks 文档：https://rollupjs.org/configuration-options/#output-manualchunks

延伸阅读：想深入源码了解 chunk 拆分算法，可从 Rolldown 的 chunking 相关文档入手（https://rolldown.rs/）；体积分析还可以配合 `size-limit` 在 CI 中做自动化预算检查。
