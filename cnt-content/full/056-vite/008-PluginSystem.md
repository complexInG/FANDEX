...
**题目**：打开 `node_modules/@vitejs/plugin-vue`（或 `plugin-react`）的源码，找出它实现了哪些钩子，用一句话说明每个钩子的作用。

**提示**：看插件入口文件的 `return { name: ..., ... }` 结构；重点找 `transform`、`handleHotUpdate`、`config` 三个钩子。

**参考答案要点**：`config` 钩子补充 vue 相关编译配置；`transform` 钩子把 `.vue` 单文件组件编译为 JS 模块；`handleHotUpdate` 钩子为 SFC 提供组件级 HMR 边界。你会发现：官方插件的写法和你第 5 节写的虚拟模块插件结构完全一致。

### 练习 2：实现虚拟模块插件（入门）

**题目**：实现一个 `virtual:version` 虚拟模块，导出当前构建版本号（可用 `process.env.npm_package_version` 或固定字符串），并在页面中显示。

**提示**：照抄第 5 节的 `resolveId` + `load` 模板；`load` 里用模板字符串生成 `export const version = '...'`。
...
export function versionPlugin(): Plugin {
  ...
  return {
    ...
    load(source) {
      if (source === resolved) {
        return `export const version = ${JSON.stringify('1.2.0')}`
      }
    },
  }
}
...
页面里 `import { version } from 'virtual:version'` 即可使用，版本号变更只改插件一处。

### 练习 3：transform 注入构建时间（进阶）

**题目**：写一个插件，在构建（build）模式下给 `main.ts` 注入一个 `__BUILD_TIME__` 常量，值为构建发生的时刻，并在页面中打印。

**提示**：`configResolved` 中判断 `command === 'build'`；`transform` 中用 `replace` 把代码里的 `__BUILD_TIME__` 替换为 `JSON.stringify(new Date().toISOString())`。
...
export function buildTimePlugin(): Plugin {
  ...
  return {
    ...
    transform(code, id) {
      if (!isBuild || !id.endsWith('main.ts')) return null
      // 替换占位符为构建时刻（每次构建都不同）
      return code.replace(
        '__BUILD_TIME__',
...
源码中写 `console.log('构建时间', __BUILD_TIME__)`，构建后会自动变成实际时间。

### 练习 4：给 markdown 文件写一个 loader（进阶）

**题目**：实现插件，让 `.md` 文件可以被直接 `import` 为字符串内容（类似 Vite 内置的 `?raw`，但用插件实现），并在页面渲染。

**提示**：`load` 钩子中判断 `id.endsWith('.md')`，用 Node 的 `fs.readFileSync` 读取文件内容并返回 `export default \`...\`` 字符串；注意模板字符串转义。

**参考答案要点**：

```ts
// plugins/md-loader.ts
...
### 练习 5（挑战）：handleHotUpdate 自定义 HMR 边界

**题目**：写一个插件：当项目里的 `data/config.json` 变化时，只触发依赖它的模块热更新（而不是全量刷新），并在热更新后重新读取配置。

**提示**：`handleHotUpdate(ctx)` 中判断 `ctx.file` 是否以 `config.json` 结尾；返回 `ctx.modules`（受影响模块列表）告诉 Vite 更新范围；配合 `import.meta.hot.accept()` 使用。

**参考答案要点**：

```ts
// plugins/config-hmr.ts
import type { Plugin } from 'vite'

export function configHmr(): Plugin {
  return {
    name: 'config-hmr',
    handleHotUpdate(ctx) {
      // 只关心 config.json 的变更
      if (!ctx.file.endsWith('config.json')) return
      // 返回受影响模块，Vite 只更新这些模块
      const affected = [...ctx.modules]
      // 通知浏览器重新拉取
      ctx.server.ws.send({ type: 'full-reload' })  // 简化处理：业务模块配合 accept 后改为精准更新
      return affected
    },
  }
}
...
提示：真实实现中，业务模块通过 `import.meta.hot.accept('./config.json', cb)` 声明接受 config 更新，插件用 `handleHotUpdate` 精准返回模块集即可——把 006 篇学的 HMR API 与本文的钩子机制串起来，就是完整的自定义 HMR 链路。
...
- Vite 插件 API（官方中文）：https://cn.vitejs.dev/guide/api-plugin
- Vite 插件 API（英文）：https://vite.dev/guide/api-plugin
- Rolldown 插件开发指南：https://rolldown.rs/apis/plugin-api
- 官方插件目录 registry.vite.dev：https://registry.vite.dev/

延伸阅读：调试插件用 `vite-plugin-inspect`（https://github.com/antfu/vite-plugin-inspect）；想了解 Vite 内部 30+ 内置插件如何组织，读 Vite 源码 `packages/vite/src/node/plugins/` 目录是最高效的路径。
