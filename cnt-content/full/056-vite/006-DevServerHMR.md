...
### 练习 1：观察一次完整的 HMR 过程（入门）

**题目**：在 `my-hmr-demo` 项目中，分别修改一个 CSS 文件和一个 TS 文件，用开发者工具回答：每次修改后页面是否刷新？WS 消息内容是什么？

**提示**：打开 DevTools 的 Network 面板，勾选 WS 标签；再打开 Console 标签，看是否有页面 reload 日志；观察修改 CSS 时页面是否闪烁。

**参考答案要点**：修改 CSS 时 WS 消息类型为 `style-update`（或对应更新），页面不刷新，样式直接生效；修改 TS 时消息类型为 `js-update`，且终端输出 `hmr update /src/main.ts`。若发现修改 TS 后页面整体刷新，说明该模块没有热更新边界——正是练习 3 要解决的。

### 练习 2：配置代理解决接口跨域（入门）

**题目**：假设后端接口在 `http://localhost:8080/api/users`，前端请求统一走 `/api` 前缀。请写出 `vite.config.ts` 的代理配置，让 `fetch('/api/users')` 能成功访问后端。

**提示**：需要 `target`、`changeOrigin`、`rewrite` 三个选项；思考 `/api/users` 转发后应变成什么路径。
...
注意 `rewrite` 去掉 `/api` 前缀的前提是后端接口路径不含 `/api`；若后端接口本身就有 `/api` 前缀，则不需要 `rewrite`。

### 练习 3：让工具模块支持热更新（进阶）

**题目**：写一个 `theme.ts` 模块，导出配色对象 `colors`；修改 `colors` 后页面样式立即更新且不刷新页面。要求：模块自身接受热更新，并在 `accept` 回调中把新颜色应用到 CSS 变量。

**提示**：用 `import.meta.hot.accept()` 接受自身更新；回调中通过 `document.documentElement.style.setProperty` 更新 CSS 变量。

**参考答案要点**：

```ts
// theme.ts
export const colors = { primary: '#3b82f6', secondary: '#8b5cf6' }

if (import.meta.hot) {
  import.meta.hot.accept((newModule) => {
    if (newModule) {
      // 把新配色同步到 CSS 变量，页面立即生效
      document.documentElement.style.setProperty('--primary', newModule.colors.primary)
      document.documentElement.style.setProperty('--secondary', newModule.colors.secondary)
      console.log('主题已热更新：', newModule.colors)
    }
  })
}
```

### 练习 4：用 dispose 防止定时器叠加（进阶）

**题目**：页面中有个 `setInterval` 每秒输出一次日志。连续热更新 5 次模块后，观察控制台每秒输出的日志条数——如果越来越多，说明定时器泄漏了。用 `hot.dispose` 修复。

**提示**：把 `timer` 声明在模块作用域；`dispose` 回调里 `clearInterval(timer)`。
...
if (import.meta.hot) {
  import.meta.hot.dispose(() => {
    clearInterval(timer) // 旧模块的定时器必须清理
  })
  import.meta.hot.accept() // 声明接受热更新
}
```

修复后，热更新任意多次，每秒都只输出一条日志。

### 练习 5（挑战）：复现"状态丢失"并定位原因

**题目**：在页面输入框里输入文字，然后修改模块中无关的代码。如果页面刷新导致输入内容丢失，分析是哪个环节的问题，并写出解决方案。

**提示**：从"模块有没有 accept 边界""是否缺少框架插件""修改的模块处于依赖链的哪个位置"三个角度排查。

**参考答案要点**：若使用 React 且输入框在组件内，安装 `@vitejs/plugin-react` 后 Fast Refresh 会保留 state；若是原生 JS 模块，需在模块内用 `hot.data` 保存输入内容并在 `accept` 回调中恢复。核心思路：**状态要放在热更新边界之内**（组件内或模块的 `hot.data` 里），而不是依赖整页不刷新。
...
- Vite 开发服务器选项（官方）：https://cn.vitejs.dev/config/server-options
- Vite HMR API（官方）：https://cn.vitejs.dev/guide/api-hmr
- Vite 服务器选项（英文）：https://vite.dev/config/server-options
- Vite 中文文档首页：https://cn.vitejs.dev/

延伸阅读：想深入了解模块图与 HMR 的内部实现，可以读 Vite 源码中的 `packages/vite/src/node/server/moduleGraph.ts` 与 `packages/vite/src/node/server/hmr.ts`；调试 HMR 问题时，在 `pnpm dev` 前加 `DEBUG=vite:hmr` 环境变量可查看 HMR 详细日志（Windows PowerShell 用 `$env:DEBUG="vite:hmr"; pnpm dev`）。
