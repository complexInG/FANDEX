...
- 评估中的权衡核心（CAP），见本模块《CAP理论与最终一致性》
- 评估前的架构设计，见本模块《软件架构概述》《领域驱动设计》
- 评估后的架构落地，见本模块《分层架构》《微服务架构》《事件驱动架构》

> **一句话记忆**：架构评估是"买二手车前的验车"——在架构定型前用质量属性场景检验它、识别风险与权衡点（ATAM），再用成本收益分析决定改什么（CBAM），最后把发现变成可执行的行动项；架构评估的习惯比评估的规格更重要。
