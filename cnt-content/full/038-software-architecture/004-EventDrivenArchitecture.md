...
- 事件驱动的数据一致性权衡，见本模块《CAP理论与最终一致性》
- 事件驱动的性能与可用性考量，见本模块《质量属性》
- 事件驱动的落地方法（领域事件），见本模块《领域驱动设计》

> **一句话记忆**：事件驱动是"订杂志"而不是"打电话"——生产者发布事实（事件）、消费者订阅自己关心的部分；事件溯源"不存结果存过程"、CQRS"读写分家各优化"，二者配合让系统既松耦合又可审计，但代价是最终一致与更高的复杂度。
