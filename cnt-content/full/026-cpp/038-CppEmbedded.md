...
**解析讲解**：C
**解析讲解**：`volatile` 告诉编译器该变量可能被外部（如硬件、中断）修改，每次访问必须从内存读取，不可缓存到寄存器或合并访问。`const` 是只读，`static` 是存储期，`register` 在 C++17 已废弃。
...
**解析讲解**：B
**解析讲解**：Cortex-M3 的 LDR/STR 指令对 4 字节对齐的 32 位访问是原子的。64 位访问需要两条指令，可能被中断撕裂。非对齐访问不保证原子。读-改-写不是单指令。
...
**解析讲解**：B
**解析讲解**：`-fno-rtti` 禁用 RTTI，移除 `typeid` 与 `dynamic_cast` 的运行时类型信息，减少代码体积。`-O3` 增大体积，`-fexceptions` 启用异常（增大），`-g3` 增加调试信息。
...
**解析讲解**：B
**解析讲解**：C++ 全局对象的构造在 `main()` 之前由启动代码调用 `__libc_init_array`（或类似函数）完成，该函数遍历 `.init_array` 段中的构造函数指针。若禁用此机制，需避免全局非平凡对象。
...
**解析讲解**：D（在标准 C++20 起）
**解析讲解**：C++20 起 `std::atomic<T>` 的 `T` 不能是 `volatile`（弃用并最终移除）。`std::atomic` 已包含必要的内存序保证，`volatile` 多余且语义冲突。C++17 之前虽允许但强烈不推荐。
...
**解析讲解**：B
**解析讲解**：SysTick 是 Cortex-M 核心自带的系统定时器异常，IRQn = -1，属于系统异常。其优先级由 SCB->SHP[11] 控制，独立于 NVIC。
...
**解析讲解**：C
**解析讲解**：`new`/`delete` 时间不确定（堆碎片化、合并耗时），违反实时性。`constexpr` 与模板是编译期，无运行时开销。`enum class` 无开销。
...
**解析讲解**：A
**解析讲解**：`> RAM` 指定 VMA（运行时地址）在 RAM，`AT > FLASH` 指定 LMA（加载地址）在 Flash。启动代码需从 LMA 拷贝到 VMA。
