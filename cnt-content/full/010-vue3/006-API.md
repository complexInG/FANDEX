...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《组合式 API》，属于 Vue 3 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《组合式 API》纳入自己的知识网络，并与 Vue 3 模块的其他主题（组合式 API、响应式、组件通信、路由、状态管理）建立关联。

## 2. 历史动机与发展脉络

《组合式 API》是 Vue 3 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：组合式 API 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《组合式 API》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《组合式 API》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
该示例来自原文《2.1 基本用法》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：2.2 script setup 语法糖

该示例来自原文《2.2 script setup 语法糖》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 ref》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.2 reactive》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.3 computed》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.4 watch》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.7 示例：3.5 watchEffect

该示例来自原文《3.5 watchEffect》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.8 示例：4. 生命周期钩子 | Lifecycle Hooks

该示例来自原文《4. 生命周期钩子 | Lifecycle Hooks》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.9 示例：5. 组合函数 | Composables

该示例来自原文《5. 组合函数 | Composables》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6.1 provide》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6.2 inject》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：7. 模板引用 | Template Refs

该示例来自原文《7. 模板引用 | Template Refs》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《8.1 toRefs》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《8.2 toRef》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《8.3 unref》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《8.4 isRef》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《10.1 计数器示例》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《10.2 表单示例》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 60 行有效代码，包含 3 类关键结构（class、import、from）。其中：
...
该示例来自原文《响应式状态》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《计算属性》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《侦听器》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《工具函数》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《依赖注入》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《模板引用》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《组件通信 API》小节，用于演示组合式 API相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《组合式 API》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《组合式 API》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《组合式 API》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. 组合式 API 概述 | Composition API Overview：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. setup 函数 | Setup Function：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 响应式 API | Reactive APIs：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 生命周期钩子 | Lifecycle Hooks：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. 组合函数 | Composables：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. 依赖注入 | Dependency Injection：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 7. 模板引用 | Template Refs：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 8. 响应式工具 | Reactive Utilities：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 9. 最佳实践 | Best Practices：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 10. 示例 | Examples：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 11. 小结 | Summary：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 延伸阅读：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 响应式状态：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 计算属性：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 侦听器：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 工具函数：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 依赖注入：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 模板引用：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 组件通信 API：该小节围绕组合式 API展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 Vue 3 模块。为了把《组合式 API》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《组合式 API》及 Vue 3 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
