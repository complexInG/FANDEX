...
```text
第 1 阶段（语法基础）：002 程序结构 → 003 变量与类型 → 004 运算符 → 005 控制流
第 2 阶段（函数与对象）：016 函数与闭包（前 5 节）→ 026 对象与数组 → 032 原型链
第 3 阶段（DOM 交互）：027 DOM 操作与事件
第 4 阶段（异步入门）：030 异步编程（回调 → Promise → async/await）
第 5 阶段（数据流通）：019 网络请求（Fetch 基础）→ 029 模块化（只读 import/export）
第 6 阶段（工程能力）：036 ES6+ → 038 防抖节流 → 039 数组高阶方法
