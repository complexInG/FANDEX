...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《测试与工程化》，属于 React 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《测试与工程化》纳入自己的知识网络，并与 React 模块的其他主题（组件、Hooks、状态管理、渲染性能）建立关联。

## 2. 历史动机与发展脉络

《测试与工程化》是 React 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：测试与工程化 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《测试与工程化》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《测试与工程化》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
该示例来自原文《1.1 安装配置》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.2 组件测试》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《1.3 异步组件测试》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 32 行有效代码，包含 2 类关键结构（import、from）。其中：
...
该示例来自原文《1.4 Hook 测试》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《2.1 安装配置》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：2.2 编写 E2E 测试

该示例来自原文《2.2 编写 E2E 测试》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 安装》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.16 示例：3.2 编写 Story

该示例来自原文《3.2 编写 Story》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.18 示例：3.3 组件测试（交互测试）

该示例来自原文《3.3 组件测试（交互测试）》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.1 安装配置》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.22 示例：5.1 GitHub Actions

该示例来自原文《5.1 GitHub Actions》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5.2 质量门禁》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.24 示例：6.1 功能模块化结构（推荐）

该示例来自原文《6.1 功能模块化结构（推荐）》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6.3 导入顺序》小节，用于演示测试与工程化相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《测试与工程化》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《测试与工程化》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《测试与工程化》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. Vitest 与 Testing Library：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. E2E 测试（Playwright）：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. Storybook：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. ESLint / Prettier：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. CI/CD：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. 项目结构最佳实践：该小节围绕测试与工程化展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 React 模块。为了把《测试与工程化》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《测试与工程化》及 React 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
