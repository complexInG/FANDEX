...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《Concurrent模式》，属于 React 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《Concurrent模式》纳入自己的知识网络，并与 React 模块的其他主题（组件、Hooks、状态管理、渲染性能）建立关联。

## 2. 历史动机与发展脉络

《Concurrent模式》是 React 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：Concurrent模式 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《Concurrent模式》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《Concurrent模式》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
### 5.1 示例：useTransition 标记非紧急更新

该示例来自原文《useTransition 标记非紧急更新》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：useDeferredValue 延迟更新

该示例来自原文《useDeferredValue 延迟更新》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.3 示例：Suspense 与并发渲染

该示例来自原文《Suspense 与并发渲染》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.4 示例：Suspense 与多数据源

该示例来自原文《Suspense 与多数据源》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 23 行有效代码，包含 5 类关键结构（class、function、import、from、return）。其中：
...
### 5.5 示例：useTransition 与列表过滤

该示例来自原文《useTransition 与列表过滤》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 26 行有效代码，包含 4 类关键结构（function、import、from、return）。其中：
...
该示例来自原文《标签页切换》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《流式 SSR》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.8 示例：Suspense 与错误边界结合

该示例来自原文《Suspense 与错误边界结合》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.9 示例：Selective Hydration

该示例来自原文《Selective Hydration》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.10 示例：useTransition 与乐观更新

该示例来自原文《useTransition 与乐观更新》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.11 示例：useTransition 过渡更新

该示例来自原文《useTransition 过渡更新》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.14 示例：startTransition 全局函数

该示例来自原文《startTransition 全局函数》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.15 示例：useDeferredValue 延迟值

该示例来自原文《useDeferredValue 延迟值》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.17 示例：Suspense 数据等待

该示例来自原文《Suspense 数据等待》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.19 示例：Suspense 配合 lazy

该示例来自原文《Suspense 配合 lazy》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《并发更新优先级》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.22 示例：避免不必要的 loading

该示例来自原文《避免不必要的 loading》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《并发渲染可中断》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.24 示例：useSyncExternalStore 订阅外部

该示例来自原文《useSyncExternalStore 订阅外部》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.25 示例：useSyncExternalStore 订阅外部
...
该示例来自原文《自动批处理》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.28 示例：flushSync 强制同步

该示例来自原文《flushSync 强制同步》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.29 示例：selectNode 调度提示

该示例来自原文《selectNode 调度提示》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.30 示例：Offscreen 隐藏组件

该示例来自原文《Offscreen 隐藏组件》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.31 示例：useOptimistic 乐观更新

该示例来自原文《useOptimistic 乐观更新》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.32 示例：Suspense List 协调多个 Suspense

该示例来自原文《Suspense List 协调多个 Suspense》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.33 示例：React 19 Actions

该示例来自原文《React 19 Actions》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《并发模式注意事项》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.35 示例：transition 与 Suspense 配合

该示例来自原文《transition 与 Suspense 配合》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《性能权衡》小节，用于演示Concurrent模式相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《Concurrent模式》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《Concurrent模式》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《Concurrent模式》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 概述：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 基础概念：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 快速上手：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 详细用法：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 常见场景：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 注意事项：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 进阶用法：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- useTransition 过渡更新：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- startTransition 全局函数：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- useDeferredValue 延迟值：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- Suspense 数据等待：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- Suspense 配合 lazy：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 并发更新优先级：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 避免不必要的 loading：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 并发渲染可中断：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- useSyncExternalStore 订阅外部：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 自动批处理：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- flushSync 强制同步：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- selectNode 调度提示：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- Offscreen 隐藏组件：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- useOptimistic 乐观更新：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- Suspense List 协调多个 Suspense：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- React 19 Actions：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 并发模式注意事项：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- transition 与 Suspense 配合：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 性能权衡：该小节围绕Concurrent模式展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 React 模块。为了把《Concurrent模式》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
| Concurrent模式 | 013-ConcurrentMode | 本文自身 |
...
下表整理《Concurrent模式》及 React 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
