...
- [第 1 章 概述与学习目标](#第-1-章-概述与学习目标)
- [第 2 章 事务理论基础](#第-2-章-事务理论基础)
- [第 3 章 PostgreSQL MVCC 实现原理](#第-3-章-postgresql-mvcc-实现原理)
- [第 4 章 隔离级别详解](#第-4-章-隔离级别详解)
- [第 5 章 锁机制深度剖析](#第-5-章-锁机制深度剖析)
- [第 6 章 快照与可见性](#第-6-章-快照与可见性)
- [第 7 章 Serializable 隔离级别实现](#第-7-章-serializable-隔离级别实现)
- [第 8 章 事务日志与 WAL](#第-8-章-事务日志与-wal)
- [第 9 章 参数调优](#第-9-章-参数调优)
- [第 10 章 性能分析与基准测试](#第-10-章-性能分析与基准测试)
- [第 11 章 最佳实践](#第-11-章-最佳实践)
- [第 12 章 常见陷阱与反模式](#第-12-章-常见陷阱与反模式)
- [第 13 章 故障排查实战](#第-13-章-故障排查实战)
- [第 14 章 对比分析](#第-14-章-对比分析)
- [第 15 章 练习题与参考答案](#第-15-章-练习题与参考答案)
- [第 16 章 参考文献](#第-16-章-参考文献)
...
本章提供 10 道覆盖理论与实操的练习题，旨在检验读者对 PostgreSQL 事务与并发控制的综合理解。
...
```
场景: 系统监控表 monitors, 字段 active 标记是否启用。
不变式: 至少有 1 个监控处于 active 状态。
两个管理员同时将各自负责的监控关闭。
```
...
MySQL InnoDB 采用原地更新 + undo log：UPDATE 直接修改数据页中的行，旧版本数据被写入独立的 undo log 回滚段。读旧版本时需通过 undo log 重建。

| 对比项 | PostgreSQL（追加式） | InnoDB（原地 + undo） |
| :--- | :--- | :--- |
...
作用：避免每次可见性判断都查询 pg_xact 提交日志，显著降低 I/O 与 CPU 开销。

纯 SELECT 也可能产生脏页的原因：当 SELECT 首次访问某元组且 Hint Bits 未设置时，PostgreSQL 会查询 pg_xact 确认 xmin/xmax 状态，确认后将结果写入 Hint Bits 并将页面标记为脏。后续的检查点会将该脏页刷盘。因此即使是只读查询，也可能产生写入 I/O。
...
```
1. 检查 xmin 的可见性:
   - 若 xmin 未提交 (查 pg_xact) -> 不可见 (除非是当前事务自身)
   - 若 xmin 已回滚 -> 不可见
   - 若 xmin 已提交:
     - 若 xmin 在快照的 xmin 之后 -> 不可见 (事务在快照之后开始)
     - 若 xmin 在快照的 xmin 之前, 但在 xip (活跃事务列表) 中 -> 不可见
     - 若 xmin 在快照的 xmin 之前, 且不在 xip 中 -> 可见 (继续检查 xmax)
     - 若 xmin == 当前事务 ID:
       - 若 cmin >= 快照的 curcid -> 不可见 (命令在快照之后)
       - 否则 -> 可见 (继续检查 xmax)
...
2. 若 xmin 可见, 检查 xmax:
   ...
   - 若 xmax 未提交或已回滚 -> 可见
   - 若 xmax 已提交:
     - 若 xmax 在快照的 xmin 之前, 且不在 xip 中 -> 不可见 (已被删除)
     - 若 xmax 在快照之后或活跃中 -> 可见 (删除对快照不可见)
   - 若 xmax 是 MultiXact -> 检查锁状态
...
```sql
-- 建表与初始数据
CREATE TABLE monitors (id int PRIMARY KEY, name text, active boolean);
INSERT INTO monitors VALUES (1, 'CPU监控', true), (2, '内存监控', true);

-- 会话 T1: 关闭 CPU 监控 (REPEATABLE READ, 会发生写偏斜)
BEGIN ISOLATION LEVEL REPEATABLE READ;
SELECT count(*) FROM monitors WHERE active = true;  -- 2, 可关闭
UPDATE monitors SET active = false WHERE id = 1;
COMMIT;

-- 会话 T2: 关闭内存监控 (与 T1 并发)
BEGIN ISOLATION LEVEL REPEATABLE READ;
SELECT count(*) FROM monitors WHERE active = true;  -- 2, 可关闭
UPDATE monitors SET active = false WHERE id = 2;
COMMIT;

-- 结果: 全部关闭, 违反不变式 (写偏斜)

-- 避免方案: 使用 SERIALIZABLE
-- 会话 T1
BEGIN ISOLATION LEVEL SERIALIZABLE;
SELECT count(*) FROM monitors WHERE active = true;  -- 2
UPDATE monitors SET active = false WHERE id = 1;
-- T2 同时执行相同逻辑
COMMIT;  -- T1 成功
...
-- 会话 T2
BEGIN ISOLATION LEVEL SERIALIZABLE;
SELECT count(*) FROM monitors WHERE active = true;  -- 2 (快照)
UPDATE monitors SET active = false WHERE id = 2;
COMMIT;
-- T2 会收到: ERROR: could not serialize access due to read/write dependencies
-- 应用层捕获错误并重试
```
...
SELECT
    ...
    age(relfrozenxid) AS xid_age,
    CASE
        WHEN age(relfrozenxid) > 1500000000 THEN '高危'
        WHEN age(relfrozenxid) > 1000000000 THEN '警告'
        WHEN age(relfrozenxid) > 500000000  THEN '关注'
        ELSE '正常'
    END AS risk_level,
...
SELECT datname, age(datfrozenxid) AS db_xid_age,
       2000000000 - age(datfrozenxid) AS xids_until_forced_readonly
FROM pg_database
...
SELECT id, balance FROM accounts
WHERE id IN ($from_id, $to_id)
ORDER BY id  -- 始终按 id 升序加锁
FOR UPDATE;
UPDATE accounts SET balance = balance - $amount WHERE id = $from_id;
UPDATE accounts SET balance = balance + $amount WHERE id = $to_id;
COMMIT;

-- 方案 2: 单条原子 UPDATE (适合简单两方转账)
UPDATE accounts
SET balance = CASE id
    WHEN $from_id THEN balance - $amount
    WHEN $to_id THEN balance + $amount
...
-- 方案 3: 锁超时 + 应用层重试 (适合高并发, 允许少量失败)
SET lock_timeout = '2s';
BEGIN;
UPDATE accounts SET balance = balance - $amount WHERE id = $from_id;
UPDATE accounts SET balance = balance + $amount WHERE id = $to_id;
COMMIT;
-- 应用层捕获 lock_timeout 错误, 指数退避重试
```
...
```
T1 --(rw)--> T2 --(rw)--> T3
   T1 先读, T2 后写 (T1 读取的数据被 T2 写入)
   T2 先读, T3 后写 (T2 读取的数据被 T3 写入)
```

即存在两条相邻的 rw-conflict，方向为 T1 -> T2 -> T3。

为什么这是冲突？假设存在串行顺序 T1 -> T2 -> T3，则 T2 的写应在 T1 的读之后（满足 rw），T3 的写应在 T2 的读之后（满足 rw）。但 T1 和 T3 之间可能存在 wr 或 ww 依赖（若 T3 写了 T1 读的数据），导致 T3 必须在 T1 之前，形成环。因此 T1 -> T2 -> T3 的 rw 依赖链无法对应任何串行顺序，构成不可序列化冲突。
...
-- 1. 获取锁 (带超时, 使用 pg_try_advisory_lock)
...
BEGIN
    LOOP
        acquired := pg_try_advisory_lock(lock_key);
        EXIT WHEN acquired;
        EXIT WHEN extract(epoch FROM (clock_timestamp() - start_time)) * 1000 > timeout_ms;
        PERFORM pg_sleep(0.05);  -- 50ms 轮询
    END LOOP;
    RETURN acquired;
END;
$$ LANGUAGE plpgsql;
...
SELECT
    ...
    granted,
    query,
    now() - query_start AS hold_duration
FROM pg_locks
WHERE locktype = 'advisory'
ORDER BY grantee;

-- 使用示例
-- 会话 A: SELECT acquire_lock(12345);  -- true
-- 会话 B: SELECT acquire_lock(12345, 3000);  -- 等待 3s 后返回 false
...
| 取值 | 行为 | 持久性 | 性能 | 推荐场景 |
| :--- | :--- | :--- | :--- | :--- |
| on | 等待本地 WAL 刷盘 + 备库收到（若同步复制） | 最高 | 最低（每次提交等待 fsync） | 金融、核心交易系统 |
| remote_write | 等待本地刷盘 + 备库写入 OS 缓存（未 fsync） | 高 | 中 | 同城灾备，可接受极小丢失窗口 |
| remote_apply | 等待本地刷盘 + 备库回放完成 | 最高 | 较低 | 读负载均衡，要求备库数据一致 |
| local | 仅等待本地 WAL 刷盘，不等备库 | 高 | 中 | 单机或异步复制，备库延迟可接受 |
| off | 不等待刷盘，异步刷盘（延迟 3 倍 wal_writer_delay） | 低 | 最高 | 日志、监控、可容忍少量丢失 |
...
-- 步骤 4: 检查备库 hot_standby_feedback (若适用)
SELECT * FROM pg_stat_replication;
...
-- 5.3 执行手动 VACUUM
VACUUM (ANALYZE) problem_table;

-- 5.4 若严重膨胀, 使用 pg_repack 在线重组
-- pg_repack -t problem_table -h localhost -U postgres
