...
|                                                                  |
|  +-----------+    +-----------+    +-----------+    +-----------+ |
|  | 客户端会话 |    | 客户端会话 |    | 客户端会话 |    | 客户端会话 | |
|  +-----+-----+    +-----+-----+    +-----+-----+    +-----+-----+ |
|        |                |                |                |       |
|        v                v                v                v       |
|  +-----+----------------+----------------+----------------+-----+ |
|  |                    共享缓冲池 (Shared Buffers)               | |
|  |                    +  WAL 缓冲区 (WAL Buffer)               | |
|  +-----+----------------+----------------+----------------+-----+ |
|        |                                                         |
|        v                                                         |
|  +-----+---------------------------------------------------------+ |
|  |                    后端进程 (Backend Processes)                | |
|  |  +----------+  +----------+  +----------+  +-------------+    | |
|  |  | Checkpo- |  | Walwriter|  | Background|  | Autovacuum  |    | |
|  |  | inter    |  |          |  | Writer    |  | Launcher    |    | |
|  |  +----------+  +----------+  +----------+  +------+------+    | |
|  |                                                  |             | |
|  |                                                  v             | |
|  |                                  +---------------+----------+  | |
|  |                                  | Autovacuum Worker x N    |  | |
|  |                                  | (执行 VACUUM / ANALYZE)  |  | |
|  |                                  +--------------------------+  | |
|  +-----+---------------------------------------------------------+ |
|        |                                                         |
...
|  |                    数据文件 (Data Files)                       | |
|  |  +----------+  +----------+  +----------+  +-------------+    | |
|  |  | 堆表文件 |  | 索引文件 |  | FSM 文件 |  | VM 文件     |    | |
|  |  | (Heap)   |  | (Index)  |  |          |  | (Visibility)|   | |
|  |  +----------+  +----------+  +----------+  +-------------+    | |
|  +---------------------------------------------------------------+ |
+------------------------------------------------------------------+
```
...
|  +--------+--------+--------+--------+--------+  |
|                                                  |
|  特殊数据区 (pd_special 之后，索引专用)           |
+--------------------------------------------------+

说明：
- pd_lower: ItemId 数组末尾偏移（空闲空间起始）
- pd_upper: 最后一个元组起始偏移（空闲空间结束）
- 空闲空间 = pd_upper - pd_lower
- 元组从页面尾部向前增长，ItemId 从页面头部向后增长
```
...
执行：INSERT INTO t VALUES (1);

执行后（假设事务 ID = 100）：
  页面:
  +---------------------------------------+
  | 元组0: t_xmin=100, t_xmax=0, data=1  |  <- 活元组
  +---------------------------------------+
```
...
执行：DELETE FROM t WHERE id = 1;  (事务ID=200)

执行后：
  页面:
  +---------------------------------------+
  | 元组0: t_xmin=100, t_xmax=200, data=1|  <- 死元组(事务200提交后)
  +---------------------------------------+

事务200提交后：
  元组0 的 t_infomask 设置 HEAP_XMAX_COMMITTED
  -> 对所有新事务不可见 -> 成为死元组
```
...
执行：UPDATE t SET id = 2 WHERE id = 1;  (事务ID=300)

执行后（HOT更新，同一页面）：
  页面0:
  +---------------------------------------+
  | 元组0: t_xmin=100, t_xmax=300, data=1|  <- 旧版本(事务300提交后死)
  | 元组1: t_xmin=300, t_xmax=0, data=2  |  <- 新版本(活元组)
  +---------------------------------------+
  元组0.t_ctid -> 元组1（指向更新后的新版本）
...
+------------------------------------------------------------------+
|                                                                  |
|  阶段1: 初始化 (initializing)                                    |
|  | - 获取 SHARE UPDATE EXCLUSIVE 锁                              |
|  | - 计算 OldestXmin 与 freeze 截止值                            |
|  v                                                               |
|  阶段2: 扫描堆表 (scanning heap)                                 |
|  | - 逐页扫描堆表，识别死元组                                     |
|  | - 收集死元组的行指针到死元组数组                               |
|  | - 维护可见性映射 (VM)                                          |
|  | - 对全可见页面执行 FREEZE 操作                                 |
|  v                                                               |
|  阶段3: 索引清理 (vacuuming indexes)                             |
|  | - 遍历所有索引，删除指向死元组的索引项                         |
|  | - 可能使用并行索引清理 (PG13+)                                |
|  v                                                               |
|  阶段4: 清理死元组 (cleaning up indexes)                         |
|  | - 实际从堆表中移除死元组                                       |
|  | - 更新 FSM (空闲空间映射)                                     |
|  v                                                               |
|  阶段5: 截断末尾空页 (truncating)                                |
|  | - 尝试获取 ACCESS EXCLUSIVE 锁                                |
|  | - 截断表末尾的全空页面，返回空间给 OS                          |
|  v                                                               |
|  阶段6: 最终清理 (performing final cleanup)                      |
|  | - 清理索引残余                                                |
|  | - 更新统计信息                                                |
|  v                                                               |
|  阶段7: 事务提交 (committing)                                    |
...
+------------------------------------------------------------------+
|                                                                  |
|  1. 获取 ACCESS EXCLUSIVE 锁（阻塞所有并发访问）                  |
|     |                                                            |
|     v                                                            |
|  2. 创建新的堆表文件                                             |
|     |                                                            |
|     v                                                            |
|  3. 顺序扫描旧表，将所有活元组写入新表（紧凑排列）                |
|     | - 同时重建所有索引                                         |
|     v                                                            |
|  4. 将新表文件替换旧表文件（文件句柄交换）                        |
|     |                                                            |
|     v                                                            |
|  5. 释放旧表文件占用的磁盘空间给 OS                               |
|     |                                                            |
|     v                                                            |
|  6. 释放 ACCESS EXCLUSIVE 锁                                     |
|                                                                  |
+------------------------------------------------------------------+
```
...
页面修剪前：
+---------------------------------------+
| Header | ItemId[0] | ItemId[1] | ItemId[2] | Free | Tup2 | Tup1 | Tup0(dead) |
|        |  -> Tup0  |  -> Tup1  |  -> Tup2  |      |      |      |             |
+---------------------------------------+

页面修剪后：
+---------------------------------------+
| Header | ItemId[0] | ItemId[1] | ItemId[2](unused) | Free(变大) | Tup2 | Tup1 |
|        |  -> Tup1  |  -> Tup2  |                    |            |      |      |
+---------------------------------------+

说明：
- ItemId[2] 被标记为 UNUSED（行指针保留但空间释放）
- Tup0 的空间被回收，并入空闲空间
- ItemId[0] 和 [1] 被重定向或调整
- 注意：行指针数组不会缩小，但元组数据区空间被回收
```
...
每位含义（2位/页面）：
  bit 0 (低位): all-visible (1=全可见)
  bit 1 (高位): all-frozen  (1=全冻结)

示例：
  页面0: 00 = 非全可见、非全冻结  -> VACUUM 需扫描
  页面1: 11 = 全可见且全冻结      -> VACUUM 跳过
  页面2: 10 = 全可见但非全冻结    -> VACUUM 需扫描(执行冻结)
  页面3: 00 = 非全可见            -> VACUUM 需扫描
  页面4: 11 = 全可见且全冻结      -> VACUUM 跳过
```
...
```
    ...
    |  4096 |     |  2048 |     |  8192 |     |  1024 |     |  512  |
    +---+---+     +---+---+     +---+---+     +---+---+     +---+---+
        |             |             |             |             |
   ...叶子节点(对应实际页面)...

每个节点存储其子树中最大的可用空间字节数。
查找时，从根节点开始，选择第一个空间 >= 请求大小的子节点向下走。
```
...
+------------------------------------------------------------------+
|                                                                  |
|  autovacuum launcher (常驻)                                      |
|  | - 每 autovacuum_naptime 秒被唤醒                              |
|  | - 检查所有数据库的统计信息                                     |
|  | - 为需要 VACUUM/ANALYZE 的表创建工作项                        |
|  | - 按 autovacuum_max_workers 限制启动 worker                   |
|  v                                                               |
|  +-------------------+  +-------------------+  +-------------+   |
|  | autovacuum worker |  | autovacuum worker |  | ... (最多N个)|  |
|  | - 对单个表执行    |  | - 对单个表执行    |  |              |  |
|  |   VACUUM/ANALYZE  |  |   VACUUM/ANALYZE  |  |              |  |
|  | - 受成本延迟限制  |  | - 受成本延迟限制  |  |              |  |
|  +-------------------+  +-------------------+  +-------------+   |
|                                                                  |
|  N = autovacuum_max_workers (默认 3)                             |
|  PG17+: autovacuum_worker_slots 预留槽位数 (默认 16)             |
|                                                                  |
+------------------------------------------------------------------+
```
...
+------------------------------------------------------------------+
|                                                                  |
|  第1层: 常规 autovacuum (阈值触发)                                |
|  | - n_dead_tup > vacuum_threshold 触发普通 VACUUM               |
|  | - 顺带冻结年龄 > vacuum_freeze_min_age 的元组                 |
|  v                                                               |
|  第2层: 急切冻结 (vacuum_freeze_table_age 触发)                  |
|  | - relfrozenxid 年龄 > 1.5亿 时全表扫描冻结                    |
|  | - 推进 relfrozenxid                                           |
|  v                                                               |
|  第3层: 强制 autovacuum (autovacuum_freeze_max_age 触发)         |
|  | - relfrozenxid 年龄 > 2亿 立即启动 worker                     |
|  | - 即使 autovacuum=off 也强制运行                              |
|  | - 不受 cost_delay 限制（最高优先级）                          |
|  v                                                               |
|  第4层: 只读保护 (XID 年龄接近 2^31)                             |
|  | - 当 XID 年龄距 2^31 仅剩 1百万时                             |
|  | - 数据库强制进入只读模式                                      |
|  | - 阻止新事务获取 XID                                          |
|  | - 仅允许执行 FREEZE 的 autovacuum                             |
|  v                                                               |
|  第5层: 启动保护 (XID 年龄极接近 2^31)                           |
|    - 单用户模式启动强制 VACUUM                                    |
|    - 极端情况下需要 --single-user 模式修复                       |
|                                                                  |
+------------------------------------------------------------------+
```
...
|                                                                  |
|  步骤1: 监控现状                                                  |
|  | - 收集 pg_stat_user_tables 数据                               |
|  | - 检测表膨胀率与死元组累积速率                                |
|  | - 检查 last_autovacuum 频率                                   |
|  v                                                               |
|  步骤2: 识别问题                                                  |
|  | - 死元组累积 > 阈值? -> 降低 scale_factor                    |
|  | - autovacuum 频繁但死元组少? -> 提高 threshold                |
|  | - autovacuum 运行太慢? -> 提高 cost_limit / 降低 cost_delay   |
|  | - autovacuum 影响业务? -> 降低 cost_limit / 提高 cost_delay   |
|  | - 事务ID接近回卷? -> 检查 freeze 参数与长事务                 |
|  v                                                               |
|  步骤3: 制定方案                                                  |
|  | - 优先按表调优（ALTER TABLE SET）                             |
|  | - 全局参数保持保守                                             |
|  | - 记录变更基线                                                 |
|  v                                                               |
|  步骤4: 实施与验证                                                |
|  | - 逐步调整，每次只改一个参数                                  |
|  | - 观察 1-2 个 naptime 周期的效果                              |
|  | - 记录前后对比数据                                             |
|  v                                                               |
|  步骤5: 迭代优化                                                  |
|  | - 根据效果持续调整                                             |
...
+------------------------------------------------------------------+
|                                                                  |
|  症状: 死元组堆积 / 表膨胀 / 性能下降 / 回卷告警                  |
|                                                                  |
|  步骤1: 确认症状                                                  |
|  | - 查询 pg_stat_user_tables 确认死元组数量                    |
|  | - 查询 pg_class 确认 XID 年龄                                 |
|  v                                                               |
|  步骤2: 检查 OldestXmin 阻塞源                                    |
|  | - 查询 pg_stat_activity 找长事务                              |
|  | - 查询 pg_replication_slots 找废弃槽                          |
|  | - 查询 pg_prepared_xacts 找未决预备事务                       |
|  v                                                               |
|  步骤3: 检查 autovacuum 配置                                      |
|  | - 确认 autovacuum=on 且 track_counts=on                      |
|  | - 检查表级 reloptions 是否禁用 autovacuum                     |
|  | - 计算实际触发阈值是否合理                                    |
|  v                                                               |
|  步骤4: 检查 autovacuum worker 状态                               |
|  | - 查看 worker 数量是否耗尽                                    |
...
|                                                                  |
|  1. 创建影子表（与原表结构相同）                                  |
|     CREATE TABLE repack_table_xxx AS SELECT ...                  |
|                                                                  |
|  2. 创建影子表的索引（与原表相同）                                 |
|     CREATE INDEX ... ON repack_table_xxx                         |
|                                                                  |
|  3. 安装触发器，记录原表的增量变更                                 |
|     CREATE TRIGGER repack_trigger ...                            |
|     -> 变更写入日志表                                             |
|                                                                  |
|  4. 将原表数据 COPY 到影子表（紧凑排列）                          |
|     INSERT INTO repack_table_xxx SELECT ... FROM original        |
|                                                                  |
|  5. 应用日志表中的增量变更到影子表                                 |
|     -> 将触发器记录的变更重放到影子表                             |
|                                                                  |
|  6. 短暂获取 ACCESS EXCLUSIVE 锁（毫秒级）                        |
|     -> 重命名影子表为原表名                                       |
|     -> 重命名原表为备份表名                                       |
|     -> 释放锁                                                     |
|                                                                  |
...
PostgreSQL 的 MVCC 采用多版本堆表模型，UPDATE/DELETE 不原地
修改数据，而是写入新版本并保留旧版本。旧版本在不再被任何活跃
事务可见后成为死元组。

死元组从产生到被 VACUUM 回收，需满足以下条件：
1. 删除/更新该元组的事务已提交（t_xmax 事务状态为 COMMITTED）。
2. 所有可能看到该元组的活跃事务都已结束（t_xmax < OldestXmin）。
3. VACUUM 扫描到该页面并执行 HeapTupleSatisfiesVacuum 判断。
4. 该页面未被标记为 all-frozen（否则 VACUUM 可能跳过）。

**答案2：**

可见性映射有两个标志位：
- all-visible：该页所有元组对所有活跃事务可见。
- all-frozen：该页所有元组已被冻结。

对 VACUUM 的影响：VACUUM 可以跳过 all-visible 且 all-frozen
的页面，大幅减少 I/O，提升扫描速度。

对仅索引扫描的影响：查询优化器执行仅索引扫描时，通过 VM 检查
索引项对应的堆页面是否 all-visible。如果是，则无需回表检查
可见性，直接使用索引数据，显著加速查询。

**答案3：**

（1）32 位事务 ID 取值范围为 0~2^32-1（约42亿），高吞吐系统可能
在数月内耗尽。XID 回卷后使用模 2^31 比较，如果元组的 t_xmin 与
当前 XID 距离超过 2^31，会被误判为"未来事务"导致数据不可见。

（2）FREEZE 将元组的 t_xmin 替换为 FrozenTransactionId（值为2），
该特殊值对所有事务可见，不再依赖模运算比较，从而免疫回卷问题。

（3）autovacuum_freeze_max_age 是防止回卷的最后防线。当任何表的
relfrozenxid 年龄达到此值时，autovacuum 立即（不等 naptime）
启动 worker 执行冻结 VACUUM，即使 autovacuum=off 也强制运行。

**答案4：**

（1）VACUUM 触发阈值 = 50 + 0.2 * 5000000 = 1,000,050

（2）死元组超过 1,000,050 时触发 autovacuum。

（3）调整后阈值 = 50 + 0.05 * 5000000 = 250,050
死元组超过 250,050 即触发，触发频率提升约 4 倍。

**答案5：**

五个维度的差异：
1. 锁级别：标准 VACUUM 用 SHARE UPDATE EXCLUSIVE（允许并发读写）；
   VACUUM FULL 用 ACCESS EXCLUSIVE（阻塞所有访问）。
2. 空间回收：标准 VACUUM 标记可重用不收缩文件；VACUUM FULL 物理
   收缩返回空间给 OS。
3. 索引处理：标准 VACUUM 清理索引项；VACUUM FULL 完全重建索引。
4. 执行速度：标准 VACUUM 快；VACUUM FULL 慢。
5. 并发影响：标准 VACUUM 低；VACUUM FULL 高（全程锁表）。

生产环境应避免频繁使用 VACUUM FULL，因为它需要 ACCESS EXCLUSIVE
锁，整个执行期间表完全不可读写，对大表可能持续数小时，严重影响
业务连续性。应通过 autovacuum 调优预防膨胀，已膨胀时使用 pg_repack
在线消除。

**答案6：**

```sql
-- 该表每秒产生10000死元组，每日约8.6亿死元组
-- 2000万行，默认阈值 = 50 + 0.2*20000000 = 4000050
-- 默认设置需积累400万死元组才触发，约7分钟
-- 但默认cost_delay=2ms使VACUUM很慢，跟不上产生速度

ALTER TABLE high_traffic_table SET (
    -- 降低scale_factor到1%，触发阈值为20万
    autovacuum_vacuum_scale_factor = 0.01,
    autovacuum_vacuum_threshold = 10000,
    -- 降低cost_delay加速VACUUM
    autovacuum_vacuum_cost_delay = '0.5ms',
    -- 提高cost_limit增加吞吐
...
-- 理由：
-- scale_factor=0.01: 阈值=10000+0.01*20000000=210000
--   约21秒触发一次，匹配写入速率
-- cost_delay=0.5ms: 几乎不休眠，最大化清理速度
-- cost_limit=2000: 高配额，允许快速I/O
-- 建议同时增大autovacuum_work_mem到512MB加速索引清理
```
...
FROM pg_stat_user_tables
WHERE n_live_tup > 100000                              -- 活元组>10万
  AND n_dead_tup > 0.2 * (n_live_tup + n_dead_tup)     -- 死元组占比>20%
  AND (
      last_autovacuum IS NULL                           -- 从未autovacuum
      OR last_autovacuum < now() - interval '1 hour'   -- 超过1小时
  )
ORDER BY n_dead_tup DESC;
```
...
WHERE xmin IS NOT NULL
ORDER BY xmin ASC;
-- 如果发现active=false且lag很大的复制槽，即为根因

-- 步骤3: 检查预备事务（题目已给出）
-- 步骤4: 检查standby反馈（hot_standby_feedback）
SELECT * FROM pg_stat_replication;

-- 步骤5: 检查是否有其他持有xmin的后端
SELECT pid, backend_xmin, backend_type, application_name
FROM pg_stat_activity
WHERE backend_xmin IS NOT NULL
ORDER BY backend_xmin;

-- 步骤6: 如果以上都无发现，检查是否有复制连接
-- 持有xmin但未在pg_stat_activity中显示
SELECT * FROM pg_stat_replication;
```

最可能的根因是废弃的复制槽（active=false 但持有旧 xmin）。
...
- 监控 XID 年龄，1.5亿告警，1.8亿紧急告警
- 定期检查废弃复制槽
- 确保所有表 autovacuum_enabled=true

**答案10：**

方案：
```sql
-- 1. 工具选择: pg_repack（有主键，7x24，需在线操作）
...
-- 2.3 低峰期执行pg_repack
-- pg_repack -d mydb -t big_table -j 4
-- -j 4: 使用4个并行worker加速

-- 2.4 监控进度
SELECT * FROM pg_stat_progress_create_index;
-- pg_repack内部使用CREATE INDEX，可通过此视图监控

-- 3. 风险评估:
-- - 需要约350GB额外磁盘空间（影子表+索引）
-- - 执行期间原表有触发器开销，写入延迟可能增加10-20%
-- - 最后重命名阶段需要短暂ACCESS EXCLUSIVE锁（毫秒级）
-- - 如果增量日志过大，重放可能耗时

-- 4. 回滚计划:
-- 4.1 如果pg_repack失败，影子表会自动清理
-- 4.2 如果手动需要回滚:
SELECT repack.repack_abort('big_table');
-- 4.3 确保有完整备份可恢复
...
通过以上十道练习题的实操训练，读者应能独立完成生产环境中常见的 VACUUM 监控、调优、故障排查与膨胀治理工作。建议在测试库中反复演练，熟练掌握每一条诊断 SQL 与处置流程。

---

## 第十五章 参考文献

本章节汇总本文撰写过程中参考的官方文档、学术论文、技术专著、社区文章与扩展工具资料，便于读者深入延伸阅读。所有引用按照类别分组列出，并附简要内容说明，帮助读者快速定位所需资料。
