...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《分区表》，属于 PostgreSQL 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《分区表》纳入自己的知识网络，并与 PostgreSQL 模块的其他主题（MVCC、窗口函数、扩展生态、高可用）建立关联。

## 2. 历史动机与发展脉络

《分区表》是 PostgreSQL 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
该示例来自原文《2. 范围分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 10 行有效代码，包含 2 类关键结构（CREATE、FROM）。其中：
...
该示例来自原文《3. 列表分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4. 哈希分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5. 分区管理》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《范围分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 3 行有效代码，包含 2 类关键结构（CREATE、FROM）。其中：
...
该示例来自原文《列表分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《哈希分区》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 4 行有效代码，包含 3 类关键结构（class、SELECT、FROM）。其中：
...
该示例共 3 行有效代码，包含 3 类关键结构（class、SELECT、FROM）。其中：
...
该示例来自原文《分区索引》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《分区裁剪》小节，用于演示分区表相关操作。阅读时请先看代码结构，再看其后的讲解。
...
- 1. 声明式分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. 范围分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. 列表分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. 哈希分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. 分区管理：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. 分区裁剪：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 范围分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 列表分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 哈希分区：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 分区管理：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 分区索引：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 分区裁剪：该小节围绕分区表展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 PostgreSQL 模块。为了把《分区表》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《分区表》及 PostgreSQL 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
...
《分区表》属于 PostgreSQL 模块。要理解它，先要理解它在模块中的位置：它解决的是该领域的一个具体问题，并依赖模块内若干前置概念；反过来，它又为后续进阶主题提供基础。
