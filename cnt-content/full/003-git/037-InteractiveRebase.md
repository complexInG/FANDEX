...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《交互式rebase》，属于 Git 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《交互式rebase》纳入自己的知识网络，并与 Git 模块的其他主题（提交、分支、合并、远程协作）建立关联。

## 2. 历史动机与发展脉络

《交互式rebase》是 Git 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：交互式rebase 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《交互式rebase》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《交互式rebase》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
### 5.1 示例：1.1 启动交互式 rebase

该示例来自原文《1.1 启动交互式 rebase》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例共 19 行有效代码，包含 1 类关键结构（for）。其中：
...
该示例来自原文《2.2 操作流程》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.5 示例：2.3 非交互式 reword

该示例来自原文《2.3 非交互式 reword》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.2 操作流程》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.9 示例：3.3 squash 顺序

该示例来自原文《3.3 squash 顺序》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.2 使用场景》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.12 示例：4.3 fixup -C 保留后者的信息

该示例来自原文《4.3 fixup -C 保留后者的信息》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.13 示例：4.4 自动 fixup 标记

该示例来自原文《4.4 自动 fixup 标记》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5.2 操作方式》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6.1 拆分流程》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《6.2 修改提交内容》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《7.1 冲突解决流程》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《7.2 跳过或中止》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.23 示例：7.3 使用 git rerere 复用冲突解决方案

该示例来自原文《7.3 使用 git rerere 复用冲突解决方案》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《8.1 黄金法则》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.25 示例：8.2 强制推送的替代方案

该示例来自原文《8.2 强制推送的替代方案》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.26 示例：8.3 rebase 前备份

该示例来自原文《8.3 rebase 前备份》小节，用于演示交互式rebase相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《交互式rebase》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《交互式rebase》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《交互式rebase》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. 交互式 rebase 基础：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. reword — 修改提交信息：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. squash — 合并提交：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. fixup — 静默合并：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. drop — 删除提交：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. edit — 拆分提交：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 7. rebase 冲突处理：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 8. 安全实践：该小节围绕交互式rebase展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 Git 模块。为了把《交互式rebase》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《交互式rebase》及 Git 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
...
本节以“把知识讲给他人听”的方式，把《交互式rebase》的核心概念重新串讲一遍。与前文按章节展开不同，这里的叙述更接近课堂总结：先说整体，再逐个展开，最后收束。

《交互式rebase》属于 Git 模块。要理解它，先要理解它在模块中的位置：它解决的是该领域的一个具体问题，并依赖模块内若干前置概念；反过来，它又为后续进阶主题提供基础。
