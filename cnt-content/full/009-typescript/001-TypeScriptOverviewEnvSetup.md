...
本模块有 55 篇文档，其中约一半是进阶与理论内容。**零基础只需要一条路径，一次只走一步**：

```text
新手最小路径（只有这一条，按顺序读）：
001 概述 → 002 基础类型 → 003 接口别名 → 004 函数泛型
    → 005 类型推断 → 008 类型守卫 → 009 索引签名 → 013 声明文件
    → 039 工程化 → 050 类型安全 API 项目
```

读完这条路径就能写真实项目；之后再按需回头补 006/007/010-012 等篇章。

> 为什么要"不按编号顺序读"：文档按**主题深度**编号，而不是按学习顺序编号。比如 015-019 是类型论进阶、022-026 是类型体操、051 是理论串讲——它们编号靠前，但内容要求先有项目经验。记住一句话：**编号小 ≠ 先读**。
...
```text
第 1 阶段（类型基础）：001 概述 → 002 基础类型 → 003 接口别名 → 004 函数泛型
    → 005 字面量与类型推断 → 006 类与装饰器
第 2 阶段（类型进阶）：008 类型守卫 → 009 索引签名 → 010 映射类型 → 013 声明文件
第 3 阶段（类型体操）：015 高级类型 → 017 协变逆变 → 022 工具类型原理 → 023 条件类型 → 046 类型体操
