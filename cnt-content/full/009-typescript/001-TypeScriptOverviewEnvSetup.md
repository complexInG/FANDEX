...
本模块有 59 篇文档：前 4 篇是新增的零基础前篇（变量、函数、类、泛型基础），从第 5 篇起进入完整体系。**零基础只需要一条路径，一次只走一步**：

```text
新手最小路径（只有这一条，按顺序读）：
001 变量基础 → 002 函数基础 → 003 类基础 → 004 泛型基础
    → 005 概述与环境 → 006 基础类型系统 → 007 接口别名 → 008 函数泛型
    → 009 类型推断 → 012 类型守卫 → 013 索引签名 → 017 声明文件
    → 043 工程化 → 054 类型安全 API 项目
```

读完这条路径就能写真实项目；之后再按需回头补 010/011/014-016 等篇章。

> 为什么要"不只看编号"：前 4 篇是零基础前篇，其后文档按**主题深度**组织，而不是严格按学习顺序编号。比如 019-023 是类型论进阶、026-030 是类型体操、055 是理论串讲——它们编号靠前，但内容要求先有项目经验。记住一句话：**编号小 ≠ 先读**。

**遇到数学符号怎么办（统一规则）**：部分进阶文档会出现形如 `Γ ⊢ e : τ` 的类型论记号、集合论与范畴论术语。零基础第一遍的规则只有一条——**看到公式直接跳过，只看代码示例、表格与"动手试试"**。这些符号是给有项目经验后回来精读的人看的，跳过不影响主线。

进阶参考：四阶段全景图（暂时不用记，需要时再回来看）：

```text
第 1 阶段（类型基础）：001 变量基础 → 002 函数基础 → 003 类基础 → 004 泛型基础
    → 005 概述与环境 → 006 基础类型系统 → 007 接口别名 → 008 函数泛型
    → 009 字面量与类型推断 → 010 类与装饰器
第 2 阶段（类型进阶）：012 类型守卫 → 013 索引签名 → 014 映射类型 → 017 声明文件
第 3 阶段（类型体操）：019 高级类型 → 021 协变逆变 → 026 工具类型原理 → 027 条件类型 → 050 类型体操
第 4 阶段（工程实践）：043 工程化 → 052 tsconfig → 054 类型安全 API 项目 → 058 工程模板
```

其中前两阶段为【核心必读】；第三、四阶段与理论串讲（051）为【进阶原理】，有项目经验后再读。
