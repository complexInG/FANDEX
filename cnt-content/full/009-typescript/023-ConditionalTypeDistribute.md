...
**解析讲解**：分发后 `T` 被替换为联合的单个成员，丢失了"是否为联合"的信息。引入 `C` 保存原始类型，内层 `[C] extends [T]` 检查原始联合是否是单个成员的子类型。

**思考题 10.8**：为什么 `IsAny` 使用 `0 extends 1 & T` 而不是 `T extends any`？

**解析讲解**：`any extends any` 会触发分发（`any` 被视为联合），结果为 `boolean` 而非 `true`。而 `0 extends 1 & T` 中，`1 & any = any`，`0 extends any` 为 `true`；对于非 `any` 类型，`1 & T` 不包含 `0`，结果为 `false`。
