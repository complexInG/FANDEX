...
本节按照布鲁姆教育目标分类学组织学习路径。本文主题为《半连接与反半连接》，属于 SQL 模块，读者可以根据自身阶段选择阅读深度。
...
通过本节学习，读者应当能够把《半连接与反半连接》纳入自己的知识网络，并与 SQL 模块的其他主题（DDL/DML、查询、索引、事务）建立关联。

## 2. 历史动机与发展脉络

《半连接与反半连接》是 SQL 领域的重要主题。要真正理解它，需要先了解它解决的问题与演进过程。
...
回到本文主题：半连接与反半连接 的提出与成熟，正是上述技术背景下的必然产物。早期实现往往以简单可用为目标，随着工程规模扩大，社区逐渐沉淀出标准做法与最佳实践；理解这一脉络，可以帮助读者判断“为什么文档中的推荐写法是现在这个样子”，也能在遇到历史遗留代码时准确识别其设计年代与取舍。


## 3. 形式化定义与核心概念精讲

本节把《半连接与反半连接》涉及的核心概念以“定义 + 讲解”的形式展开。读者应把定义当作工具，把讲解当作理解路径；两者结合才能形成可迁移的知识。
...
本节深入《半连接与反半连接》背后的原理。理论部分不求面面俱到，而是聚焦“能解释现象、能指导实践”的关键推导。
...
### 5.1 示例：2.1 EXISTS 语法

该示例来自原文《2.1 EXISTS 语法》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.2 示例：2.2 NOT EXISTS 语法

该示例来自原文《2.2 NOT EXISTS 语法》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.3 示例：2.3 EXISTS 的特点

该示例来自原文《2.3 EXISTS 的特点》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《3.1 IN 子查询》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.5 示例：3.2 NOT IN 的 NULL 陷阱

该示例来自原文《3.2 NOT IN 的 NULL 陷阱》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.2 优化器转换》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《4.3 选择建议》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《5.2 查看执行计划》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.9 示例：6.2 LEFT JOIN + IS NULL 模式

该示例来自原文《6.2 LEFT JOIN + IS NULL 模式》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.10 示例：7.1 关联 EXISTS 实现分组过滤

该示例来自原文《7.1 关联 EXISTS 实现分组过滤》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.11 示例：7.2 双重 NOT EXISTS（关系除法）

该示例来自原文《7.2 双重 NOT EXISTS（关系除法）》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.12 示例：半连接（Semi Join）

该示例来自原文《半连接（Semi Join）》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.15 示例：反连接（Anti Join）

该示例来自原文《反连接（Anti Join）》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
### 5.18 示例：IN 与 EXISTS 对比

该示例来自原文《IN 与 EXISTS 对比》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《半连接/反连接应用场景》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
该示例来自原文《性能优化》小节，用于演示半连接与反半连接相关操作。阅读时请先看代码结构，再看其后的讲解。
...
对比是理解《半连接与反半连接》定位的最快路径。下面从多个维度与相邻方案进行对比。
...
本节把《半连接与反半连接》放入真实工程场景，给出可复用的模式与组织方法。
...
本节通过一个完整案例把《半连接与反半连接》的知识串起来。案例按“需求分析、方案设计、实现、验证”四步展开。
...
- 1. 半连接与反半连接概念：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 2. EXISTS 与 NOT EXISTS：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 3. IN 与 NOT IN：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 4. EXISTS vs IN 性能对比：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 5. 半连接的执行计划：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 6. 反半连接的执行计划：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 7. 高级应用：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 半连接（Semi Join）：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 反连接（Anti Join）：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- IN 与 EXISTS 对比：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 半连接/反连接应用场景：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
- 性能优化：该小节围绕半连接与反半连接展开具体细节，阅读时应关注其与核心结论的对应关系；每个小节都是核心结论在某一个侧面的展开。
...
本文属于 SQL 模块。为了把《半连接与反半连接》放入完整的知识网络，下面列出本模块的全部主题并给出相互关联的导读。学习时建议按模块内顺序推进，并在每个文档中留意交叉引用。
...
下表整理《半连接与反半连接》及 SQL 模块中出现的高频术语，给出简明释义。术语按字母序或逻辑序排列，供查阅。
