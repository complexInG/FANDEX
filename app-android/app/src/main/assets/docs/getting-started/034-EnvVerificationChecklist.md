...
related:
  ...
  - 'getting-started/035-TroubleshootingGuide'
prerequisites:
  - 'getting-started/020-WindowsEnvConfigTutorial'
  - 'getting-started/023-NodeJsInstall'
  - 'getting-started/027-PythonInstall'
  - 'getting-started/032-GitInstallConfig'
---

## 0. 这份清单怎么用

打开终端（Windows 用 PowerShell 或 WSL，macOS/Linux 用自带终端），逐条执行下面的命令。每一条都有“预期结果”；如果输出与预期不符，跳到 `getting-started/035-TroubleshootingGuide` 找对应问题。
...
**预期**：`node -v` 输出 `v20+` 或更高（LTS 版）；`npm -v` 输出 `10+`。若提示找不到命令，检查是否安装了 Node.js，或 PATH 是否包含 Node 目录（`getting-started/009-EnvVarPath`）。
...
**预期**：npm 输出国内镜像地址（如 `https://registry.npmmirror.com`）或官方源；pip 输出镜像配置（如有）。配置方法见 `getting-started/024-NpmManager` 与 `getting-started/028-PipVenvManager`。
...
- 环境变量与 PATH：`getting-started/009-EnvVarPath`；
- 卡住排查：`getting-started/035-TroubleshootingGuide`；
- 平台配置：`getting-started/020-WindowsEnvConfigTutorial`、`getting-started/021-MacOSEnvConfigTutorial`、`getting-started/022-LinuxEnvConfigTutorial`。
