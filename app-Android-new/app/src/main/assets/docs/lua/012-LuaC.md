...
flowchart TD
    T0["my_lua_module/"]
    T1["src/"]
    T2["my_module.c          # 主模块实现"]
    T3["my_module.h          # 内部头文件"]
    T4["compat.h             # 跨版本兼容层"]
    T5["third_party/         # 第三方依赖"]
    T6["lua/                     # Lua 测试与示例"]
    T7["test.lua"]
    T8["example.lua"]
    T9["tests/                   # C 单元测试"]
    T10["test_my_module.c"]
    T11["CMakeLists.txt           # CMake 构建脚本"]
    T12["rockspec                 # LuaRocks 包描述"]
    T13["README.md"]
    T0 --> T1
    T5 --> T6
    T8 --> T9
