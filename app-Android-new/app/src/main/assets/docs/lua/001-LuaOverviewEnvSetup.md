...
related:
  ...
  - 'lua/025-StandardLibraryDetailed'
  - 'lua/010-EnvironmentModule'
  - 'lua/012-LuaC'
prerequisites: []
---
...
flowchart TD
    ...
    T1["lua/                      -- 源码目录"]
    T2["myproject/"]
    T3["init.lua          -- 模块入口（对应 require 'myproject'）"]
    T4["core.lua          -- 核心逻辑"]
    T5["utils.lua         -- 工具函数"]
    T6["ext/              -- 扩展子模块"]
    T7["http.lua"]
    T8["main.lua              -- 主入口"]
    T9["spec/                     -- 测试目录（使用 busted）"]
    T10["core_spec.lua"]
    T11["utils_spec.lua"]
    T12["meta/                     -- 类型定义（供 lua_ls 检查）"]
    T13["myproject/"]
    T14["benchmark/                -- 性能测试"]
    T15["bench.lua"]
    T16["build/                    -- 构建产物"]
    ...
    T19["release.lua"]
    T20["docs/                     -- 文档"]
    T21[".luacheckrc               -- luacheck 配置"]
    T22[".luarc.json               -- lua_ls 配置"]
    T23["myproject-1.0.0-1.rockspec  -- LuaRocks 包描述"]
    T24["Makefile                  -- 构建 Makefile"]
    T25["README.md"]
    T0 --> T1
    T8 --> T9
    T11 --> T12
...
flowchart TD
    T0["MyAddon/"]
    T1["MyAddon.toc          -- Addon 元数据"]
    T2["MyAddon.lua          -- 主逻辑"]
    T3["MyAddon.xml          -- UI 布局"]
    T4["libs/                -- 第三方库"]
    T5["Ace3/"]
    T0 --> T1
    T0 --> T2
